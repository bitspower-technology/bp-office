...
The NiuOffice `OEM` branch itself is source-only. Do not tag it, publish an OEM
release from it, or upload OEM executables to `Niuulh/NiuOffice`. A distributor
...
4. Validate locally and in CI. A local package may be created for validation,
   but do not upload it to the NiuOffice repository.
...
For later upstream work, integrate shared changes from NiuOffice `main` into
NiuOffice `OEM` first, then merge the resulting OEM branch into the downstream
...
user-data directory can collide with NiuOffice/GenOffice installations.
...
- `apps/shell/build/generate-brand-assets.py` does **not** parse that SVG. It
  ...
  `NiuOffice` wordmark. Update or rewrite this generator for the client art.
...
```powershell
rg -n "NiuOffice AI|NiuOffice|Niuulh/NiuOffice" apps packages branding *.md package.json
```
...
may retain NiuOffice names if they are not displayed and renaming them would
add migration risk.
...
NiuOffice maintainer's certificate.
