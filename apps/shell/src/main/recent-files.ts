...
/** Keep legacy presentation entries in storage, but hide them from the shell once Slides is unavailable. */
export function withoutLegacySlidePaths(paths: readonly string[]): string[] {
  return paths.filter((path) => !/\.pptx$/i.test(path))
}
