...
import { PRODUCT_RELEASES_URL } from '../shared/product-config'
import {
  closeUpdateWindow,
  isUpdateWindowOpen,
  pushUpdateState,
...
/**
 * Full-package auto-update over the generic provider.
 *
 ...
 * itself comes from branding/product.json (or the validated
 * NIUOFFICE_UPDATE_URL packaging override).
 *
 ...
 * target (latest-linux.yml). On Linux only AppImage runs self-update (electron-updater
...
const DOWNLOAD_PAGE_URL = PRODUCT_RELEASES_URL
...
export function initAutoUpdater(getWindow: () => BrowserWindow | null): void {
  ...
  // electron-builder's Portable target cannot apply NSIS updates. Its
  // launcher sets one or both of these variables; keep Portable builds on
  // the explicit download path instead of offering an update that cannot be
  // installed safely.
  if (
  ...
  // The channel setter enables downgrades by default; updates are forward-only.
