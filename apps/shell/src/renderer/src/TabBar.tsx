...
/**
 * Tab-strip file-type marks: the same distributor-owned SVGs the Home list uses,
 * so a tab and its Home card always carry the identical BP Office badge.
 */
function BrandFileIcon({ src }: { src: string }) {
  return <img className="tab-icon-img" src={src} width={16} height={16} alt="" aria-hidden="true" />
}
...
const KIND_ICON: Record<TabSummary['kind'], ReactElement> = {
  home: <HomeIcon />,
  docs: <BrandFileIcon src={iconDocx} />,
  sheets: <BrandFileIcon src={iconXlsx} />,
  pdf: <BrandFileIcon src={iconPdf} />,
  markdown: <BrandFileIcon src={iconMd} />,
}
