...
import bpOfficeLogo from './assets/genoffice-logo.svg'
...
// ── Local AI status (bottom-left) ───────────────────────

const LM_STUDIO_POLL_MS = 10_000

function LmStudioEntry() {
  const { t } = useI18n()
  ...
  const refreshStatus = useCallback(async (config?: LmStudioConfig, showChecking = true) => {
    ...
    try {
      ...
      if (seq === requestSeq.current) setStatus(null)
    } finally {
  ...
  useEffect(() => {
    ...
    const applyTabs = (tabs: Awaited<ReturnType<typeof window.aiOfficeTabs.list>>) => {
      ...
      if (becameActive && document.visibilityState === 'visible') void refreshStatus()
    }
    ...
    const poll = window.setInterval(() => {
      if (homeActive.current && document.visibilityState === 'visible') {
        void refreshStatus(undefined, false)
      }
    }, LM_STUDIO_POLL_MS)
    const onFocus = () => {
      if (homeActive.current) void refreshStatus()
    }
    const onVisibility = () => {
      if (homeActive.current && document.visibilityState === 'visible') {
        void refreshStatus(undefined, false)
  ...
  useEffect(() => window.aiOffice.onAiSettingsChanged(() => void refreshStatus()), [refreshStatus])

  const state = checking ? 'checking' : (status?.state ?? 'unreachable')
  const statusLabel = checking
    ? t('lmStudioChecking')
    : state === 'connected'
      ? status?.selectedModel
        ? t('lmStudioConnectedModel', { model: status.selectedModel })
        : t('lmStudioConnected')
      : state === 'no-models'
        ? t('lmStudioNoModels')
        : state === 'unauthorized'
    ...
    <div className="local-ai-entry">
      {settingsOpen && (
        <SettingsModal
          ...
          onRefreshLmStudio={(config) => refreshStatus(config)}
      ...
      <button
        ...
        data-tip={status?.error || statusLabel}
        aria-label={t('lmStudioOpenSettings')}
      >
        ...
        <span className="local-ai-text">
          ...
          <span className={`local-ai-status ${state}`} data-testid="lmstudio-status-text">
            {statusLabel}
          </span>
        </span>
        <svg
      ...
      <div className="quick-cards">
        ...
        <button className="quick-card" onClick={() => void window.aiOffice.browse()}>
          ...
          <span className="quick-text">
            ...
            <span className="quick-sub">.docx / .xlsx / .xls / .csv / .pdf / .md</span>
          </span>
        </button>
      </div>
    )
