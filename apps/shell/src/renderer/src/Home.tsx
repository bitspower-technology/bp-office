...
import { chatGptPlanLabel } from './chatgpt-display'
...
// ── AI provider status (bottom-left) ────────────────────

const AI_STATUS_POLL_MS = 10_000

function AiProviderEntry() {
  const { t } = useI18n()
  const [provider, setProvider] = useState<AiConnectionProvider>('lmstudio')
  ...
  const [lmStudioChecking, setLmStudioChecking] = useState(true)
  const [chatGptChecking, setChatGptChecking] = useState(false)
  const [settingsOpen, setSettingsOpen] = useState(false)
  const requestSeq = useRef(0)
  const homeActive = useRef(false)
  const providerRef = useRef<AiConnectionProvider>('lmstudio')

  const refreshLmStudio = useCallback(async (config?: LmStudioConfig, showChecking = true) => {
    const seq = ++requestSeq.current
    if (showChecking) setLmStudioChecking(true)
    try {
      const next = await window.aiOffice.lmStudioStatus(config)
      if (seq === requestSeq.current) setLmStudioStatus(next)
    } catch {
      if (seq === requestSeq.current) setLmStudioStatus(null)
    } finally {
      if (seq === requestSeq.current) setLmStudioChecking(false)
    }
  }, [])

  const refreshChatGpt = useCallback(async (config?: ChatGptConfig, showChecking = true) => {
    const seq = ++requestSeq.current
    if (showChecking) setChatGptChecking(true)
    try {
      const next = await window.aiOffice.chatGptStatus(config)
      if (seq === requestSeq.current) setChatGptStatus(next)
    } catch {
      if (seq === requestSeq.current) setChatGptStatus(null)
    } finally {
      if (seq === requestSeq.current) setChatGptChecking(false)
    }
  }, [])

  const refreshSelected = useCallback(
    async (showChecking = true) => {
      let nextProvider = providerRef.current
      try {
        nextProvider = await window.aiOffice.getAiProvider()
      } catch {
        // A stale preload/main pair safely falls back to the local provider.
      }
      providerRef.current = nextProvider
      setProvider(nextProvider)
      if (nextProvider === 'chatgpt') await refreshChatGpt(undefined, showChecking)
      else await refreshLmStudio(undefined, showChecking)
    },
    [refreshChatGpt, refreshLmStudio],
  )

  const changeProvider = useCallback(
    async (nextProvider: AiConnectionProvider) => {
      const saved = await window.aiOffice.setAiProvider(nextProvider)
      providerRef.current = saved
      setProvider(saved)
      if (saved === 'chatgpt') await refreshChatGpt()
      else await refreshLmStudio()
    },
    [refreshChatGpt, refreshLmStudio],
  )
  ...
  useEffect(() => {
    ...
    const applyTabs = (tabs: Awaited<ReturnType<typeof window.aiOfficeTabs.list>>) => {
      ...
      const nextHomeActive = !active || active.kind === 'home'
      const becameActive = nextHomeActive && !homeActive.current
      homeActive.current = nextHomeActive
      if (becameActive && document.visibilityState === 'visible') void refreshSelected()
    }
    void window.aiOfficeTabs.list().then(applyTabs)
    const stopTabs = window.aiOfficeTabs.onChanged(applyTabs)
    const poll = window.setInterval(() => {
      if (homeActive.current && document.visibilityState === 'visible') {
        void refreshSelected(false)
      }
    }, AI_STATUS_POLL_MS)
    const onFocus = () => {
      if (homeActive.current) void refreshSelected()
    }
    const onVisibility = () => {
      if (homeActive.current && document.visibilityState === 'visible') {
        void refreshSelected(false)
  ...
  useEffect(
    () =>
      window.aiOffice.onOpenLocalAiSettings(() => {
  ...
  useEffect(
    () => window.aiOffice.onAiSettingsChanged(() => void refreshSelected()),
    [refreshSelected],
  )

  const chatGptRateLimited = chatGptStatus?.rateLimits.some((limit) => limit.reached) ?? false
  const state =
    provider === 'chatgpt'
      ? chatGptChecking
        ? 'checking'
        : chatGptStatus?.state === 'connected' && chatGptRateLimited
          ? 'rate-limited'
          : (chatGptStatus?.state ?? 'unavailable')
      : lmStudioChecking
        ? 'checking'
        : (lmStudioStatus?.state ?? 'unreachable')
  const statusLabel =
    provider === 'chatgpt'
      ? chatGptChecking
        ? t('lmStudioChecking')
    ...
    <div className="local-ai-entry">
      {settingsOpen && (
        <SettingsModal
          ...
          onRefreshLmStudio={(config) => refreshLmStudio(config)}
          onRefreshChatGpt={(config) => refreshChatGpt(config)}
          onAiProviderChange={changeProvider}
          onClose={() => setSettingsOpen(false)}
        />
      )}
      <button
        className="local-ai-btn"
        data-testid={provider === 'chatgpt' ? 'chatgpt-status-button' : 'lmstudio-status-button'}
        data-provider={provider}
        onClick={handleClick}
        aria-haspopup="dialog"
        aria-expanded={settingsOpen}
        data-tip={statusLabel}
        aria-label={t('aiProviderOpenSettings')}
      >
        <span className="local-ai-icon" aria-hidden="true">
          {provider === 'chatgpt' ? (
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
              <path
                d="M3 3.25h10v7.2H8.2L5.1 13v-2.55H3z"
                stroke="currentColor"
                strokeWidth="1.2"
                strokeLinejoin="round"
              />
            ...
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
              <rect x="2.25" y="2.5" width="11.5" height="4.25" rx="1.4" stroke="currentColor" />
              <rect x="2.25" y="9.25" width="11.5" height="4.25" rx="1.4" stroke="currentColor" />
              <path
                d="M4.5 4.65h.01M4.5 11.4h.01"
                stroke="currentColor"
                strokeWidth="1.8"
                strokeLinecap="round"
              />
          ...
          <span
            className={`local-ai-dot ${state}`}
            data-testid={provider === 'chatgpt' ? 'chatgpt-status-dot' : 'lmstudio-status-dot'}
            data-state={state}
          />
        </span>
        <span className="local-ai-text">
          <span className="local-ai-name">
            {provider === 'chatgpt' ? 'ChatGPT' : 'OpenAI Endpoint'}
          </span>
          <span
            className={`local-ai-status ${state}`}
            data-testid={provider === 'chatgpt' ? 'chatgpt-status-text' : 'lmstudio-status-text'}
          >
            {statusLabel}
          </span>
        </span>
        <svg
          className="local-ai-chevron"
          width="14"
          height="14"
          viewBox="0 0 16 16"
          fill="none"
      ...
      <div className="quick-cards">
        ...
        <button className="quick-card" onClick={() => void window.aiOffice.browse()}>
          ...
          <span className="quick-text">
            ...
            <span className="quick-sub">.docx / .xlsx / .xlsm / .xls / .csv / .pdf / .md</span>
          </span>
        </button>
      </div>
    )
