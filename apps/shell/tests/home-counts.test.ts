...
import { pageRecentPaths, withoutLegacySlidePaths } from '../src/main/recent-files'
...
describe('home visible counts', () => {
  ...
  it('excludes legacy presentation paths before paging', () => {
    const dir = mkdtempSync(join(tmpdir(), 'shell-counts-'))
    tempDirs.push(dir)
    const docPath = join(dir, 'notes.docx')
    const slidePath = join(dir, 'legacy.PPTX')
    writeFileSync(docPath, 'doc')
    writeFileSync(slidePath, 'slide')

    const page = pageRecentPaths(
      withoutLegacySlidePaths([docPath, slidePath]),
      {},
      new Set([slidePath]),
    )
