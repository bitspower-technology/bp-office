...
describe('generated BP-Office brand assets', () => {
  ...
  it('uses one valid 1024px RGBA icon for shell and renderer surfaces', () => {
    ...
    // The 0.6.1 mark: pure black tile with a white monogram.
    expect(countOpaqueColor(png, 0, 0, 0), 'missing black icon tile').toBeGreaterThan(500_000)
    expect(
      countOpaqueMatching(png, (red, green, blue) => red > 240 && green > 240 && blue > 240),
      'missing white monogram',
  ...
  it('contains the complete Windows ICO size set as 32-bit frames', () => {
    ...
    for (const frame of frames) {
      ...
      if (frame.size === 256) {
        // The repo icon stores the largest frame as an embedded PNG.
        const png = decodeRgbaPng(frame.payload)
        expect({ width: png.width, height: png.height }).toEqual({ width: 256, height: 256 })
        expect(alphaBounds(png).visiblePixels).toBeGreaterThan(0)
        continue
      }
      // Smaller frames are classic BITMAPINFOHEADER DIBs (XOR mask + AND mask).
      const andRowBytes = ((frame.size + 31) >> 5) << 2
      expect(frame.payload.length).toBe(40 + frame.size * frame.size * 4 + andRowBytes * frame.size)
      expect(frame.payload.readUInt32LE(0), 'biSize').toBe(40)
      expect(frame.payload.readInt32LE(4), 'biWidth').toBe(frame.size)
      expect(frame.payload.readInt32LE(8), 'biHeight').toBe(frame.size * 2)
      expect(frame.payload.readUInt16LE(12), 'biPlanes').toBe(1)
      expect(frame.payload.readUInt16LE(14), 'biBitCount').toBe(32)
    }
  })
  ...
  it('uses the repo BP mark from the active Home import', () => {
    const newLogoPath = join(shellRenderer, 'assets', 'bpoffice-logo.svg')
    ...
    expect(homeSource).toMatch(
      /^import bpOfficeLogo from ['"]\.\/assets\/bpoffice-logo\.svg['"]$/m,
    )
    expect(homeSource).not.toMatch(/genoffice-logo\.svg/i)

    // The BP mark from the bitspower-technology/bp-office repo (Illustrator export).
    const svg = readFileSync(newLogoPath, 'utf8')
    expect(svg).toContain('viewBox="380 380 239.833 239.833"')
  ...
  it('keeps the 0.6.1 BP-Office mark as the canonical source', () => {
    const svg = readFileSync(join(shellBuild, 'bpoffice-mark.svg'), 'utf8')
    expect(svg).toContain('BP-Office mark')
    expect(svg).toContain('<rect width="240" height="240" rx="54" fill="black"/>')
    expect(svg).toContain('fill="white"')
    expect(svg).not.toMatch(/GenOffice|Genspark/i)

    const canonical = readFileSync(repoFile('branding', 'bpoffice-gradient-outline.svg'), 'utf8')
    expect(canonical, 'canonical branding SVG must match the build copy').toBe(svg)
  })
  ...
  it('brands both Windows artifacts and excludes removed packaged modules', () => {
    ...
    expect(manifest).toMatchObject({ productName: 'BP-Office', version: '0.8.358-bp.2' })
    expect(builder).toContain("productName: 'BP-Office'")
    expect(builder).toContain("target: 'portable'")
    expect(builder).toContain("artifactName: 'BP-Office Setup ${version}.${ext}'")
    expect(builder).toContain("artifactName: 'BP-Office Portable ${version}.${ext}'")
    ...
    expect(manifest.devDependencies?.['@openai/codex']).toBe('0.147.0')
