...
describe('opening tabs', () => {
  ...
  it('binds a shell-opened workbook path to the new sheets view', () => {
    manager.openSheetsTab('/tmp/one.xlsx')
    manager.openSheetsTab('/tmp/two.xlsx')
    expect(createSheetsView.mock.calls.slice(-2)).toEqual([
      [{ includeAiHandlers: false, openPath: '/tmp/one.xlsx' }],
      [{ includeAiHandlers: false, openPath: '/tmp/two.xlsx' }],
    ])
  })
...
describe('activation', () => {
  it('recognizes only live editor WebContents owned by the manager', async () => {
    const id = manager.openPdfTab('/tmp/scan.pdf')
    const view = lastCreatedView(createPdfView)
    expect(manager.ownsWebContents(view.webContents.id)).toBe(true)
    expect(manager.ownsWebContents(999)).toBe(false)
    await manager.closeTab(id)
    expect(manager.ownsWebContents(view.webContents.id)).toBe(false)
  })
...
describe('closing tabs', () => {
  ...
  it('keeps the current tab active when closing a background tab', async () => {
    ...
    expect(manager.list().find((t) => t.id === pdfId)?.active).toBe(true)
  })
...
describe('file path bookkeeping', () => {
  ...
  it('finds tabs by kind and path', () => {
    ...
    expect(manager.findPdfTabByPath('/tmp/c.pdf')).toBe('t2')
