...
const config = {
  ...
  extraResources: [
    ...
    // Official Codex native runtime used only for managed ChatGPT OAuth and
    // app-server JSON-RPC. The package version is pinned in the shell manifest.
    {
      from: codexBinary,
  ...
  linux: {
    // Downloaded artifacts carry BP-Office branding while package/executable
    ...
    maintainer: 'BP-Office Contributors',
    vendor: 'BP-Office Contributors',
    category: 'Office',
  ...
  beforePack: async (context) => {
    ...
    assertCodexRuntimeMatches(context)
