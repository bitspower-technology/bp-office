...
export const AGENT_TOOLS: AgentToolDef[] = [
  ...
  },
  {
    name: 'insert_image',
    description:
      'Download an explicit HTTP(S) image URL supplied by the user and insert it into the document after a top-level block. The image file is saved next to the document, so the document must have been saved to disk at least once.',
    inputSchema: {
      type: 'object',
      properties: {
...
// ── explicit URL image insertion (insert_image) ──
