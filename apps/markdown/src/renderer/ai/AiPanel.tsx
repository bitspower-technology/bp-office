...
export function AiPanel({
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.markdownApi.onAiSettingsChanged((settings) => {
  ...
  if (!loopRef.current) {
    loopRef.current = new AgentLoop<DocSnapshot>({
      ...
      skill: createMarkdownSkill(() => depsRef.current.getEditor(), {
        ...
        {chat.map((entry, i) => {
          ...
          return (
            ...
            >
              ...
              {entry.isError && isLast && !busy && (
                <div className="ai-error-actions">
                  <button className="ai-retry-btn" onClick={retry}>
                    {t('aiRetry')}
                  </button>
                  <button
