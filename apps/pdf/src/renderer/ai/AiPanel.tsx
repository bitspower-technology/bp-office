...
export function AiPanel({
  ...
  useEffect(() => {
    ...
    void store
      ...
      .then((ids) => {
        ...
        return store.loadChat({ projectId: ids.projectId, chatId: ids.chatId, limit: 512 })
      })
      .then((msgs) => {
  ...
  useEffect(() => {
    ...
    void window.pdfApi.getAiSettings().then((settings) => {
      if (active) settingsRef.current = settings
    })
    const unsubscribe = window.pdfApi.onAiSettingsChanged((settings) => {
  ...
  if (!loopRef.current) {
    const deps: PdfAiDeps = {
      ...
      outlineReady: () => apiRef.current.outlineReady(),
      outline: () => apiRef.current.outline(),
      setBookmarks: (bookmarks) => apiRef.current.setBookmarks(bookmarks),
  ...
  const retryLastInstruction = (): void => {
    const prior = [...chat].reverse().find((entry) => entry.role === 'user' && entry.text.trim())
    if (prior) send(prior.text)
  }
        ...
        {chat.map((entry, i) => {
          ...
          return (
            ...
            >
              ...
              {entry.isError && i === chat.length - 1 && !busy && (
                <div className="ai-error-actions">
                  <button className="ai-retry-btn" onClick={retryLastInstruction}>
                    {t('aiRetry')}
                  </button>
                  <button
