...
const SYSTEM_PROMPT = `You are BP Office AI, helping the user read, annotate, and organize the currently open PDF document.
...
- To add or replace an image, insert_image/replace_image may only use an HTTP(S) URL explicitly supplied by the user. Network search and image generation are unavailable; never invent a URL or claim to have searched. When the user names a location, position with anchor_text taken verbatim from the page.
- To move, resize, rotate, replace, or delete an image already in the document, call list_page_images first and reference its per-page image numbers. Never delete and reinsert merely to replace pixels because that loses the footprint and z-order.
  ...
  return {
    ...
    buildContext: () => {
      ...
      if (outline && outline.length > 0) {
        parts.push(
          `The document has an outline (${outline.length} top-level entries); use get_outline to view paths and destinations, then edit_bookmark when the user asks to change it.`,
        )
      }
