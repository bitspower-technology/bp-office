...
const OUTLINE_OUTPUT_CHARS = 24_000
const OUTLINE_TITLE_CODEPOINTS = 512
const OUTLINE_TRUNCATED_NOTICE =
  '[Outline output truncated at the safety limit; later entries are not available to this tool call.]'
...
export interface PdfAiDeps {
  ...
  /** False only while the source outline is still being resolved. */
  outlineReady(): boolean
  outline(): OutlineNode[] | null
  /** Replace the complete pending internal bookmark tree (queued until Save). */
  setBookmarks(bookmarks: BookmarkInput[]): void
...
export const AGENT_TOOLS: AgentToolDef[] = [
  ...
  },
  {
    name: 'insert_image',
    description:
      'Download an HTTP(S) image URL explicitly supplied by the user and place it on a page (takes effect on save). Never invent or search for a URL. Position it either with anchor_text plus placement, or with explicit x/y in points measured from the page top-left as displayed; with neither, the image is centered on the page.',
    inputSchema: {
      type: 'object',
      properties: {
        page: { type: 'integer', description: 'Page number (1-based)' },
        url: {
          type: 'string',
          description: 'Direct image link explicitly supplied by the user',
        },
        anchor_text: {
          type: 'string',
  ...
  },
  {
    name: 'replace_image',
    description:
      "Swap an existing page image's pixels for an HTTP(S) URL explicitly supplied by the user — footprint and z-order survive and the new image stretches to the old footprint (takes effect on save). Call list_page_images first; image_number refers to that listing. Never invent or search for a URL.",
    inputSchema: {
      type: 'object',
      properties: {
        page: { type: 'integer', description: 'Page number (1-based)' },
  ...
  },
  {
    name: 'get_outline',
    description:
      'Read the document outline (bookmarks) tree, including stable hierarchy paths, entry titles, and destination pages. Use the returned path with edit_bookmark. Returns empty if the document has no outline.',
    inputSchema: { type: 'object', properties: {} },
  },
  {
    name: 'edit_bookmark',
    description:
      'Create, update, delete, or move a PDF outline bookmark (takes effect on save; undo is available). First call get_outline and use its 1-based hierarchy paths such as "2" or "2.1". Create: title + page, optional parent_path/position/bold/italic. Update: path plus at least one of title/page/bold/italic. Delete: path (deletes that bookmark and its descendants). Move: path plus optional parent_path and position (omitting parent_path moves to the top level).',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        path: {
          type: 'string',
          description: 'Existing bookmark path from get_outline, e.g. "2.1"',
        },
        title: { type: 'string', description: 'Unicode bookmark title' },
        page: { type: 'integer', description: 'Destination page number (1-based)' },
        parent_path: {
          type: 'string',
          description: 'Destination parent path; omit for the top level',
        },
        position: {
          type: 'integer',
          description: '1-based position among the destination siblings; omit to append',
        },
        bold: { type: 'boolean', description: 'Whether the bookmark title is bold' },
        italic: { type: 'boolean', description: 'Whether the bookmark title is italic' },
      },
      required: ['action'],
    },
  },
...
async function insertImageTool(
  ...
  if (!/^https?:\/\//.test(url))
    return err('invalid URL; use an HTTP(S) image URL explicitly supplied by the user', summary)
  const geom = deps.pageGeom(r.origIdx)
  if (!geom) return err('Document not ready', summary)
  const size = geomDispSize(geom)
  ...
  if (!fetched)
    return err('downloading the user-supplied image failed (it may not be accessible)', summary)
...
async function replaceImageTool(
  ...
  if (!fetched)
    return err(
      'The image could not be downloaded or decoded (the link may be inaccessible or not a raster image); ask the user for another direct HTTP(S) image link.',
      summary,
    )
...
function optionalBookmarkPath(
  input: Record<string, unknown>,
  key: 'parent_path',
): { ok: true; value?: string } | { ok: false; error: string } {
  if (input[key] === undefined || input[key] === null || input[key] === '') return { ok: true }
  if (typeof input[key] !== 'string') return { ok: false, error: `${key} must be a string` }
  const value = input[key].trim()
  return value ? { ok: true, value } : { ok: true }
}

function optionalBookmarkPosition(
  input: Record<string, unknown>,
): { ok: true; value?: number } | { ok: false; error: string } {
  if (input.position === undefined) return { ok: true }
  const value = Number(input.position)
  return Number.isInteger(value)
    ? { ok: true, value }
    : { ok: false, error: 'position must be a 1-based integer' }
}

function bookmarkBoolean(
  input: Record<string, unknown>,
  key: 'bold' | 'italic',
): { ok: true; value?: boolean } | { ok: false; error: string } {
  if (input[key] === undefined) return { ok: true }
  return typeof input[key] === 'boolean'
    ? { ok: true, value: input[key] }
    : { ok: false, error: `${key} must be a boolean` }
}

function editBookmarkTool(deps: PdfAiDeps, input: Record<string, unknown>): ToolExecution {
  const summary = t('outline')
  if (deps.readOnly()) return err(READONLY_OUTPUT, summary)
  if (!deps.outlineReady())
    return err('The document outline is still loading; retry this bookmark edit.', summary)
  const sourceOutline = deps.outline()
  const currentOutline = sourceOutline
    ? pruneOutlineForDeletedPages(sourceOutline, deps.isDeleted)
    : null
  const editable = outlineToBookmarks(currentOutline, deps.pageCount())
  if (!editable.ok) return err(editable.error, summary)

  const action = input.action
  if (action !== 'create' && action !== 'update' && action !== 'delete' && action !== 'move')
    return err('action must be one of create/update/delete/move', summary)

  const position = optionalBookmarkPosition(input)
  if (!position.ok) return err(position.error, summary)
  const parentPath = optionalBookmarkPath(input, 'parent_path')
  if (!parentPath.ok) return err(parentPath.error, summary)

  let edit: BookmarkEdit
  if (action === 'create') {
    if (typeof input.title !== 'string') return err('create requires a title', summary)
    const page = resolvePage(deps, input.page)
    if ('bad' in page) return err(page.bad, summary)
    const bold = bookmarkBoolean(input, 'bold')
    if (!bold.ok) return err(bold.error, summary)
    const italic = bookmarkBoolean(input, 'italic')
    if (!italic.ok) return err(italic.error, summary)
    edit = {
      action,
      title: input.title,
      pageIndex: page.origIdx,
      ...(parentPath.value ? { parentPath: parentPath.value } : {}),
      ...(position.value === undefined ? {} : { position: position.value }),
      ...(bold.value === undefined ? {} : { bold: bold.value }),
      ...(italic.value === undefined ? {} : { italic: italic.value }),
    }
  } else {
    if (typeof input.path !== 'string' || !input.path.trim())
      return err(`${action} requires a bookmark path from get_outline`, summary)
    const path = input.path.trim()
    if (action === 'delete') edit = { action, path }
    else if (action === 'move') {
      edit = {
        action,
        path,
      ...
      const hasBold = input.bold !== undefined
      const hasItalic = input.italic !== undefined
      if (!hasTitle && !hasPage && !hasBold && !hasItalic)
        return err('update requires at least one of title/page/bold/italic', summary)
      if (hasTitle && typeof input.title !== 'string') return err('title must be a string', summary)
      const page = hasPage ? resolvePage(deps, input.page) : undefined
      if (page && 'bad' in page) return err(page.bad, summary)
      const bold = bookmarkBoolean(input, 'bold')
      if (!bold.ok) return err(bold.error, summary)
      const italic = bookmarkBoolean(input, 'italic')
      if (!italic.ok) return err(italic.error, summary)
      edit = {
        ...
        ...(page && 'origIdx' in page ? { pageIndex: page.origIdx } : {}),
        ...(bold.value === undefined ? {} : { bold: bold.value }),
        ...(italic.value === undefined ? {} : { italic: italic.value }),
      }
    }
  ...
  const result = applyBookmarkEdit(editable.bookmarks, edit)
  if (!result.ok) return err(result.error, summary)
  deps.setBookmarks(result.bookmarks)
  return {
    output: `${result.description} (unsaved; the user can undo with ⌘Z and saves with ⌘S).`,
    mutated: true,
    summary,
  }
}
...
export async function executePdfTool(
  ...
  switch (call.name) {
    ...
    case 'delete_page': {
      ...
      if (deps.readOnly()) return err(READONLY_OUTPUT, summary)
      if (!deps.outlineReady())
        return err('The document outline is still loading; retry this page deletion.', summary)
      const r = resolvePage(deps, input.page)
      if ('bad' in r) return err(r.bad, summary)
    ...
    case 'get_outline': {
      if (!deps.outlineReady())
        return err('The document outline is still loading; retry get_outline.', t('aiToolOutline'))
      const sourceOutline = deps.outline()
      ...
      const addLine = (line: string): boolean => {
        ...
        const reserved = OUTLINE_TRUNCATED_NOTICE.length + 1
        if (outputChars + separator + line.length > OUTLINE_OUTPUT_CHARS - reserved) {
          truncated = true
          return false
        }
        lines.push(line)
        outputChars += separator + line.length
        return true
      }
      const displayTitle = (title: string): string => {
        ...
        for (const codepoint of title) {
          if (count === OUTLINE_TITLE_CODEPOINTS) return `${prefix}… [title truncated]`
          prefix += codepoint
          count++
        }
        return prefix
      }
      const walk = (nodes: OutlineNode[], depth: number, parentPath = ''): boolean => {
        for (let index = 0; index < nodes.length; index++) {
          const n = nodes[index]!
          const path = parentPath ? `${parentPath}.${index + 1}` : String(index + 1)
          const destination =
            n.pageIndex !== undefined
              ? `page ${n.pageIndex + 1}`
              : n.url
                ? 'external link'
          ...
          if (
            !addLine(
              `${'  '.repeat(depth)}[${path}] ${JSON.stringify(displayTitle(n.title))} — ${destination}`,
            )
          ) {
            return false
          }
          if (n.items && !walk(n.items, depth + 1, path)) return false
        }
        return true
      }
      if (outline) walk(outline, 0)
      if (truncated) lines.push(OUTLINE_TRUNCATED_NOTICE)
      return {
    ...
    case 'edit_bookmark':
      return editBookmarkTool(deps, input)
