...
import {
  LEGACY_VISUAL_SIGNATURE_CONTENT_PREFIX,
...
function visualSignatureFieldName(annotation: RawFormAnnotation): string | null {
  ...
  for (const prefix of [VISUAL_SIGNATURE_CONTENT_PREFIX, LEGACY_VISUAL_SIGNATURE_CONTENT_PREFIX]) {
    if (contents?.startsWith(prefix)) return contents.slice(prefix.length)
  }
  return null
}
