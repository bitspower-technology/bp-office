...
export default function App() {
  ...
  const outlineRef = useRef<OutlineNode[] | null>(null)
  const outlineReadyRef = useRef(false)
  /** Complete local replacement outline; null means preserve the source outline. */
  const [bookmarkEdits, setBookmarkEdits] = useState<BookmarkInput[] | null>(null)
  /** Synchronous mirror: consecutive AI tool calls can arrive before React rerenders. */
  const bookmarkEditsRef = useRef<BookmarkInput[] | null>(bookmarkEdits)
  bookmarkEditsRef.current = bookmarkEdits
  ...
  const deletedRef = useRef<Set<number>>(deleted)
  deletedRef.current = deleted
  const visibleOutline = useMemo(() => {
    const base = bookmarkEdits === null ? outline : bookmarksToOutline(bookmarkEdits)
    return base ? pruneOutlineForDeletedPages(base, (pageIndex) => deleted.has(pageIndex)) : null
  }, [bookmarkEdits, deleted, outline])
  ...
  const loadDoc = useCallback(
    async (
      ...
      const outlinePromise = loaded
        .getOutline()
        .then(async (value) => {
          const raw = value && value.length > 0 ? (value as OutlineNode[]) : null
          return raw ? await resolveOutlinePages(loaded, raw) : null
        })
        .catch((): OutlineNode[] => [
          {
            title: '[Outline unavailable]',
            viewError: 'outline could not be read safely',
          },
        ])
      ...
      const resolvedOutline = await outlinePromise
      outlineRef.current = resolvedOutline
      setSizes(all)
      setBaseRots(rots)
      ...
      if (!saved) {
        ...
        setOrder(null)
        bookmarkEditsRef.current = null
        setBookmarkEdits(null)
        setMetadata(null)
      } else {
        ...
        const remappedDeleted = new Set<number>()
        // Saved deletions are absent from pageMap; the rest were deleted mid-save
        for (const oldIdx of deletedRef.current) {
          const ni = remap.get(oldIdx)
          if (ni !== undefined) remappedDeleted.add(ni)
        }
        deletedRef.current = remappedDeleted
        setDeleted(remappedDeleted)
        setOrder((prev) => {
          if (!prev) return null
        ...
        const pendingBookmarks = bookmarkEditsRef.current
        const remappedBookmarks =
          pendingBookmarks === saved.bookmarks
            ? null
            : pendingBookmarks === null
              ? null
              : remapBookmarkPages(pendingBookmarks, saved.pageMap)
        bookmarkEditsRef.current = remappedBookmarks
        setBookmarkEdits(remappedBookmarks)
      }
      ...
      // Re-enable outline/page mutations only after every saved-state ref above has
      // been reconciled; otherwise a tool call can mix new outline indices with old edits.
  ...
  const snapshot = (): EditSnapshot => ({
    ...
    deleted,
    order,
    bookmarks: bookmarkEditsRef.current,
    metadata,
  })
  ...
  const applySnapshot = (s: EditSnapshot) => {
    ...
    setOrder(s.order)
    bookmarkEditsRef.current = s.bookmarks
    setBookmarkEdits(s.bookmarks)
  ...
  const undo = () => {
    // A save snapshot cannot represent an undo of operations already being written.
    // Keep undo deterministic by waiting until the post-save reconciliation finishes.
  ...
  const editsPayload = (
    ...
    ...(bookmarkEditsRef.current === null ? {} : { bookmarks: bookmarkEditsRef.current }),
    ...(metadata ? { metadata } : {}),
  })
  ...
  const deletePage = (origIdx: number) => {
    ...
    pushUndo()
    const nextDeleted = new Set(deletedRef.current).add(origIdx)
    ...
    let currentBookmarks = bookmarkEditsRef.current
    if (currentBookmarks === null && outlineRef.current) {
      const editable = outlineToBookmarks(outlineRef.current, sizes.length)
      if (editable.ok) currentBookmarks = editable.bookmarks
    }
    if (currentBookmarks !== null) {
      const next = pruneBookmarksForDeletedPage(currentBookmarks, origIdx)
      bookmarkEditsRef.current = next
      setBookmarkEdits(next)
    }
    return true
  }
  ...
  const aiPendingSummary = (): string => {
    ...
    if (bookmarkEdits !== null) parts.push('bookmark changes: pending replacement outline')
  ...
  const aiApi: PdfAiDeps = {
    ...
    outlineReady: () => outlineReadyRef.current,
    outline: () => {
      const pending = bookmarkEditsRef.current
      const base = pending === null ? outlineRef.current : bookmarksToOutline(pending)
      return base
        ? pruneOutlineForDeletedPages(base, (pageIndex) => deletedRef.current.has(pageIndex))
        : null
    },
    setBookmarks: (bookmarks) => {
      pushUndoRef.current()
      bookmarkEditsRef.current = bookmarks
      setBookmarkEdits(bookmarks)
    },
    ...
    <div className="ribbon-group">
      <div className="ribbon-group-items">
        ...
        <button
          ...
          disabled={!visibleOutline || visibleOutline.length === 0}
      ...
      <div className="ribbon">
        <div className="ribbon-tabs">
          ...
          <button
            ...
            disabled={undoStack.length === 0 || saveState === 'saving'}
            onClick={undo}
          >
            <IconUndo />
          </button>
          <button
            ...
            disabled={redoStack.length === 0 || saveState === 'saving'}
            onClick={redo}
          >
            <IconRedo />
          </button>
      ...
      <div className="app-main">
        ...
        <div className="app-content">
          <div className="pdf-body">
            {sidebar === 'outline' && visibleOutline && visibleOutline.length > 0 && (
