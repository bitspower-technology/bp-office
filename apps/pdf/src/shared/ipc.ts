...
export type PdfConvertFormat = 'docx' | 'xlsx'
...
/** Complete editable PDF outline tree. Page indices address the source document. */
export type BookmarkViewType = 'XYZ' | 'Fit' | 'FitH' | 'FitV' | 'FitR' | 'FitB' | 'FitBH' | 'FitBV'
...
export interface BookmarkInput {
  ...
  bold?: boolean
  italic?: boolean
  /** Preserve the source destination view; newly created bookmarks default to Fit. */
  view?: BookmarkView
  /** PDF outline title color as normalized RGB components. */
  color?: [number, number, number]
  /** Whether descendants are initially expanded; defaults to true. */
  expanded?: boolean
...
export interface SavePdfRequest {
  ...
  pageOrder?: number[]
  /** Complete resulting outline; omitted preserves the source outline, empty removes it. */
...
export const AI_CHANNELS = {
  ...
  openSettings: 'ai:open-local-ai-settings',
...
export interface PdfApi {
  ...
  /** Convert the current PDF to Word or Excel via the shell's local conversion flows. */
  ...
  /** Open local Explorer/Finder files as shell tabs. */
  ...
  openLocalAiSettings(): Promise<void>
  aiStream(request: AiStreamRequest): Promise<void>
  aiToolResult(request: AiToolResultRequest): Promise<void>
