...
async function addImageStamp(
  ...
  annot.set(PDFName.of('T'), PDFHexString.fromText('NiuOffice'))
...
function addDrawing(
  ...
  if (d.kind === 'note') {
    ...
    annot.set(PDFName.of('T'), PDFHexString.fromText(d.author || 'NiuOffice'))
...
export async function applySaveRequest(
  ...
  if (request.bookmarks !== undefined)
    applyBookmarks(pdfDoc, request.bookmarks, pages, request.deletedPages)
