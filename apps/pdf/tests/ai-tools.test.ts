...
import type { BookmarkInput, FormValueInput, PageImageRef } from '../src/shared/ipc'
import { bookmarksToOutline } from '../src/renderer/bookmarks'
...
describe('get_outline / unknown tools', () => {
  ...
  it('blocks reads and edits until the source outline has finished resolving', async () => {
    const deps = makeDeps({ outlineReady: () => false })
    const read = await executePdfTool(deps, call('get_outline'))
    expect(read.isError).toBe(true)
    expect(read.output).toContain('still loading')
    const edit = await executePdfTool(
      deps,
      call('edit_bookmark', { action: 'create', title: 'Too soon', page: 1 }),
    )
    expect(edit.isError).toBe(true)
    expect(edit.output).toContain('still loading')
    const deleted = await executePdfTool(deps, call('delete_page', { page: 1 }))
    expect(deleted.isError).toBe(true)
    expect(deleted.output).toContain('still loading')
    expect(deps.setBookmarks).not.toHaveBeenCalled()
    expect(deps.deletePage).not.toHaveBeenCalled()
  })

  it('creates and then updates Unicode bookmarks across consecutive tool calls', async () => {
    let bookmarks: BookmarkInput[] = [{ title: 'Existing', pageIndex: 0 }]
    const deps = makeDeps({
      outline: () => bookmarksToOutline(bookmarks),
      setBookmarks: (next) => {
        bookmarks = next
      },
    })

    const created = await executePdfTool(
      deps,
      call('edit_bookmark', {
        action: 'create',
        title: '第二章 — Résumé',
    ...
    expect(bookmarks).toEqual([
      { title: 'Existing', pageIndex: 0 },
      { title: '第二章 — Résumé', pageIndex: 1, bold: true },
    ])
    ...
    expect(bookmarks).toEqual([
      { title: '第一章', pageIndex: 1, italic: true },
      { title: '第二章 — Résumé', pageIndex: 1, bold: true },
    ])
  })

  it('deletes bookmark subtrees and moves nodes without losing metadata', async () => {
    const setBookmarks = vi.fn()
    const source: BookmarkInput[] = [
      {
        title: 'A',
        pageIndex: 0,
        view: { type: 'FitH' as const, args: [846] },
        color: [0.1, 0.2, 0.3] as [number, number, number],
        expanded: false,
        children: [{ title: 'A.1', pageIndex: 1 }],
      },
      { title: 'B', pageIndex: 1 },
    ]
    const move = await executePdfTool(
      makeDeps({ outline: () => bookmarksToOutline(source), setBookmarks }),
      call('edit_bookmark', { action: 'move', path: '2', parent_path: '1', position: 1 }),
    )
    expect(move.mutated).toBe(true)
    expect(setBookmarks).toHaveBeenCalledWith([
      {
        ...source[0],
        children: [source[1], source[0].children[0]],
      },
    ])

    setBookmarks.mockClear()
    const remove = await executePdfTool(
      makeDeps({ outline: () => bookmarksToOutline(source), setBookmarks }),
      call('edit_bookmark', { action: 'delete', path: '1' }),
    )
    expect(remove.output).toContain('Deleted bookmark 1')
    expect(setBookmarks).toHaveBeenCalledWith([source[1]])
  })

  it('rejects invalid paths, page destinations, cycles, read-only files, and lossy outlines', async () => {
    const source = [
      { title: 'A', pageIndex: 0, items: [{ title: 'A.1', pageIndex: 1 }] },
      { title: 'External', url: 'https://example.com' },
    ]
    for (const [deps, input, match] of [
      [makeDeps(), { action: 'create', title: 'Bad', page: 3 }, 'out of range'],
      [
        makeDeps({ outline: () => source.slice(0, 1) }),
        { action: 'update', path: '9', title: 'Bad' },
        'was not found',
      ],
      [
        makeDeps({ outline: () => source.slice(0, 1) }),
        { action: 'move', path: '1', parent_path: '1.1' },
        'descendants',
      ],
      [makeDeps({ readOnly: () => true }), { action: 'create', title: 'X', page: 1 }, 'read-only'],
      [
        makeDeps({ outline: () => source }),
        { action: 'update', path: '1', title: 'X' },
        'external link',
      ],
    ] as const) {
      const result = await executePdfTool(deps, call('edit_bookmark', input))
      expect(result.isError).toBe(true)
  ...
  it('hides deleted bookmark destinations and promotes surviving children', async () => {
    const outline = [
      {
        title: 'Deleted parent',
        pageIndex: 0,
        items: [{ title: 'Surviving child', pageIndex: 1 }],
      },
    ]
    const result = await executePdfTool(
      makeDeps({ outline: () => outline, isDeleted: (pageIndex) => pageIndex === 0 }),
      call('get_outline'),
    )
    expect(result.output).toBe('[1] "Surviving child" — page 2')
  })

  it('bounds hostile outline titles and total tool output', async () => {
    const result = await executePdfTool(
      makeDeps({
        outline: () =>
          Array.from({ length: 100 }, (_, index) => ({
            title: `${String(index).padStart(3, '0')}-${'x'.repeat(2_000)}`,
            pageIndex: 0,
          })),
      }),
      call('get_outline'),
    ...
    expect(result.output.length).toBeLessThanOrEqual(24_000)
    expect(result.output).toContain('[1] "000-')
    expect(result.output).toContain('… [title truncated]')
    expect(result.output).toContain('[Outline output truncated at the safety limit;')
  })
