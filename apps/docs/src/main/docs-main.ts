...
const AI_IPC_ID_MAX = 128
const AI_TOOL_OUTPUT_MAX = 2_000_000

interface PendingAiToolResult {
  readonly name: string
  resolve(result: AgentToolResult): void
  reject(error: Error): void
}

interface ActiveAiStream {
  readonly senderId: number
  readonly requestId: string
  readonly controller: AbortController
  readonly pendingTools: Map<string, PendingAiToolResult>
}

/** Sender id is part of the key: renderer-provided requestId is correlation, never authority. */
const activeAiStreams = new Map<string, ActiveAiStream>()
const aiStreamKey = (senderId: number, requestId: string) => `${senderId}:${requestId}`

function chatGptProvider() {
  const executable = process.platform === 'win32' ? 'codex.exe' : 'codex'
  return getSharedChatGptProviderService({
    codexHome: userDataPath('chatgpt', 'codex-home'),
    workingDirectory: userDataPath('chatgpt', 'workspace'),
    ...(app.isPackaged
...
function parseAiToolResultRequest(value: unknown): AiToolResultRequest {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new Error('Invalid AI tool result.')
  }
  const request = value as Record<string, unknown>
  const resultValue = request.result
  ...
  if (
    ...
    request.requestId.length > AI_IPC_ID_MAX ||
    typeof result.id !== 'string' ||
    result.id.length < 1 ||
    result.id.length > AI_IPC_ID_MAX ||
    typeof result.name !== 'string' ||
    result.name.length < 1 ||
    result.name.length > AI_IPC_ID_MAX ||
    typeof result.output !== 'string' ||
    result.output.length > AI_TOOL_OUTPUT_MAX ||
...
function rejectPendingAiTools(stream: ActiveAiStream, reason: string): void {
  for (const pending of stream.pendingTools.values()) pending.reject(new Error(reason))
  stream.pendingTools.clear()
}

function waitForAiToolResult(
  stream: ActiveAiStream,
  call: AgentToolCall,
): Promise<AgentToolResult> {
  if (stream.controller.signal.aborted) {
    return Promise.reject(new Error('The AI request was cancelled.'))
  }
  if (stream.pendingTools.has(call.id)) {
    return Promise.reject(new Error(`Duplicate AI tool call id: ${call.id}`))
  }
  return new Promise<AgentToolResult>((resolve, reject) => {
    stream.pendingTools.set(call.id, { name: call.name, resolve, reject })
  })
}
...
export function registerAiIpc(): void {
  ...
  ipcMain.handle('ai:set-settings', (_event, settings: AiSettings) => {
    const resolved = constrainAiSettingsToProduct(resolveAiSettings(settings, defaultAiSettings()))
    writeJson(SETTINGS_PATH(), resolved)
    for (const contents of webContents.getAllWebContents()) {
      if (!contents.isDestroyed()) contents.send('ai:settings-changed', resolved)
    }
  })

  ipcMain.handle('ai:stream', async (event, request: AiStreamRequest) => {
    const { requestId, system, messages } = request
    const settings = constrainAiSettingsToProduct(request.settings)
    if (typeof requestId !== 'string' || requestId.length < 1 || requestId.length > AI_IPC_ID_MAX) {
    ...
    const send = (chunk: AiStreamChunk) => {
      if (chunk.type === 'done' || chunk.type === 'error') terminalSent = true
      if (!event.sender.isDestroyed()) event.sender.send('ai:stream-chunk', chunk)
    }
    const requiresApiKey = AI_PROVIDERS.find((meta) => meta.id === provider)?.requiresApiKey ?? true
    ...
    if (provider !== 'lmstudio' && provider !== 'chatgpt' && !config.model) {
      send({ requestId, type: 'error', error: tm('errNoModel') })
      return
    }
    const key = aiStreamKey(event.sender.id, requestId)
    if (activeAiStreams.has(key)) {
      send({ requestId, type: 'error', error: 'An AI request with this id is already active.' })
      return
    }
    const controller = new AbortController()
    const active: ActiveAiStream = {
      senderId: event.sender.id,
      requestId,
      controller,
      pendingTools: new Map(),
    }
    activeAiStreams.set(key, active)
    const onSenderDestroyed = () => {
      controller.abort()
      rejectPendingAiTools(active, 'The editor was closed before the tool completed.')
      if (activeAiStreams.get(key) === active) activeAiStreams.delete(key)
    }
    ...
    try {
      if (provider === 'chatgpt') {
        await chatGptProvider().stream(request, {
          signal: controller.signal,
          onChunk: send,
          onDynamicToolCall: (call) => waitForAiToolResult(active, call),
        })
      } else {
      ...
      if (controller.signal.aborted) {
        if (!terminalSent) send({ requestId, type: 'done', stopReason: 'cancelled' })
      } else {
        send({
          requestId,
          type: 'error',
      ...
      rejectPendingAiTools(active, 'The AI request ended before the tool completed.')
      if (activeAiStreams.get(key) === active) activeAiStreams.delete(key)
      if (!event.sender.isDestroyed()) event.sender.removeListener('destroyed', onSenderDestroyed)
    }
  })

  ipcMain.handle('ai:tool-result', async (event, input: unknown) => {
    const { requestId, result } = parseAiToolResultRequest(input)
    const stream = activeAiStreams.get(aiStreamKey(event.sender.id, requestId))
    if (!stream || stream.controller.signal.aborted) {
      throw new Error('This AI tool request is no longer active.')
    }
    const pending = stream.pendingTools.get(result.id)
    if (!pending) throw new Error('Unknown or already completed AI tool request.')
    if (pending.name !== result.name)
      throw new Error('AI tool result name does not match its request.')
    stream.pendingTools.delete(result.id)
    pending.resolve(result)
    // Let the provider promise continuation write item/tool/call's response to
    // app-server before the renderer receives this IPC acknowledgement. The
    // renderer may intentionally interrupt the managed turn at that boundary.
    await new Promise<void>((resolve) => setImmediate(resolve))
  })

  ipcMain.handle('ai:stream-cancel', (event, requestId: unknown) => {
    if (typeof requestId !== 'string' || requestId.length < 1 || requestId.length > AI_IPC_ID_MAX) {
      throw new Error('Invalid AI request id.')
    }
    const stream = activeAiStreams.get(aiStreamKey(event.sender.id, requestId))
    ...
    rejectPendingAiTools(stream, 'The AI request was cancelled.')
  })
  ...
  ipcMain.handle(
    ...
    async (_event, url: string): Promise<{ base64: string; mime: string } | null> => {
      try {
        // The URL is explicitly supplied by the user. Still refuse non-http
        // schemes and private/link-local targets;
...
export function registerProjectIpc(): void {
  ...
  ipcMain.handle(
    ...
    (
      ...
      const store = getProjectStore()
      return store.loadChat(args.projectId, args.chatId, args.limit ?? 512)
    },
  )
