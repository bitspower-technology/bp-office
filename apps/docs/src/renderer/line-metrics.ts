...
export function cssFontFamily(font: string, followAltName = true): string {
  ...
  // 'NiuOffice Box Drawing' (fonts.css): U+2500 rules would otherwise fall to
  // the fullwidth CJK subset and overflow the column
  const BOX = 'NiuOffice Box Drawing'
  ...
  // 'NiuOffice PUA Blank' keeps AI-residue PUA tokens invisible like Word
  // (Calibri/Carlito would otherwise supply a box .notdef for them).
  if (f.includes('aptos')) {
    return `${chain(font, 'Calibri', 'Carlito GO', 'NiuOffice PUA Blank', CJK_SANS)},sans-serif`
  }
  ...
  // 'NiuOffice Songti SC' (fonts.css local() alias of Songti SC): macOS Chromium
  ...
  if (f.includes('simsun') || f.includes('宋体') || f.includes('nsimsun')) {
    return `${chain(font, 'NiuOffice Songti SC', 'STSong', 'SimSun', CJK_SERIF)},serif`
  }
  ...
  if (
    ...
    if (/nanum ?gothic|나눔 ?고딕/i.test(nfkc)) {
      return `${chain(font, 'NiuOffice Gothic KR', ...KO_SANS)},sans-serif`
    }
  ...
  if (/tamil|latha|vijaya|inaimathi/i.test(nfkc)) {
    return `${chain(font, 'NiuOffice Tamil', 'InaiMathi', 'Tamil MN', 'Tamil Sangam MN')},sans-serif`
  }
  ...
  const pua = latinNamed ? ['NiuOffice PUA Blank'] : []
