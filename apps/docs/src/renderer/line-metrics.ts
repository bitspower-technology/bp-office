...
export function cssFontFamily(font: string, followAltName = true): string {
  ...
  // 'BP-Office Box Drawing' (fonts.css): U+2500 rules would otherwise fall to
  // the fullwidth CJK subset and overflow the column
  const BOX = 'BP-Office Box Drawing'
  ...
  // 'BP-Office PUA Blank' keeps AI-residue PUA tokens invisible like Word
  // (Calibri/Carlito would otherwise supply a box .notdef for them).
  if (f.includes('aptos')) {
    return `${chain(font, 'Calibri', 'Carlito GO', 'BP-Office PUA Blank', CJK_SANS)},sans-serif`
  }
  ...
  // 'BP-Office Songti SC' (fonts.css local() alias of Songti SC): macOS Chromium
  ...
  if (f.includes('simsun') || f.includes('宋体') || f.includes('nsimsun')) {
    return `${chain(font, 'BP-Office Songti SC', 'STSong', 'SimSun', CJK_SERIF)},serif`
  }
  ...
  // 'BP-Office *' entries are CJK-only local() aliases (fonts.css): the underlying
  ...
  const KO_SANS = ['Malgun Gothic', 'BP-Office Sans KR', 'Apple SD Gothic Neo', 'Noto Sans KR']
  const KO_SERIF = ['BP-Office Batang', 'BP-Office Serif KR', 'BP-Office Myungjo', 'Noto Serif KR']
  const TC_SANS = ['Microsoft JhengHei', 'PingFang TC', 'BP-Office Heiti TC', 'Noto Sans TC']
  // 'BP-Office Fullwidth TC' (fonts.css): fullwidth U+FF0D/FF0F/FF3C/FF3F/FF5E whose Songti TC glyphs look half-width
  const TC_SERIF = ['BP-Office MingLiU', 'BP-Office Fullwidth TC', 'Songti TC', 'Noto Serif TC']
  const SC_SANS = ['PingFang SC', 'Microsoft YaHei', CJK_SANS]
  const SC_SERIF = ['BP-Office Songti SC', 'STSong', 'SimSun', CJK_SERIF]
  const nfkc = font.normalize('NFKC')
  ...
  if (
    ...
    if (/nanum ?gothic|나눔 ?고딕/i.test(nfkc)) {
      return `${chain(font, 'BP-Office Gothic KR', ...KO_SANS)},sans-serif`
    }
  ...
  if (/tamil|latha|vijaya|inaimathi/i.test(nfkc)) {
    return `${chain(font, 'BP-Office Tamil', 'InaiMathi', 'Tamil MN', 'Tamil Sangam MN')},sans-serif`
  }
  ...
  const pua = latinNamed ? ['BP-Office PUA Blank'] : []
