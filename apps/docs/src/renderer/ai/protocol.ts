...
export const AGENT_SYSTEM_PROMPT = [
  ...
  '- Images: insert_image may only use an HTTP(S) URL explicitly supplied by the user. Network search and image generation are unavailable; never invent a URL or claim to have searched;',
  ...
  '- Charts: use insert_chart for data visualization (bar/line/pie; saved as native Word charts); use edit_chart to change the data of an existing chart block in the block list; data must be real, from the document or values explicitly supplied by the user;',
