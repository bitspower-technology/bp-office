...
export const AGENT_TOOLS: AgentToolDef[] = [
  ...
  },
  {
    name: 'insert_image',
    description:
      'Download an HTTP(S) image URL explicitly supplied by the user and insert it into the document (at the cursor / end of document). Never invent or search for a URL.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'insert_chart',
    description:
      'Insert a chart (saved as a native Word chart). Data must come from the document or values explicitly supplied by the user — do not make up numbers.',
    inputSchema: {
      type: 'object',
      properties: {
...
/** Async tools that download an explicitly user-provided image or create a local document. */
...
export function executeTool(
  ...
  if (call.name === 'insert_image' || call.name === 'create_document') {
