...
async function mount(partial: Partial<Props>) {
  ...
  const onApply = vi.fn<(result: ProtectDialogResult) => void>(() => resolveApplied())
  ...
  // Real password creation performs 100,000 asynchronous SHA-512 iterations.
  // Await the callback instead of imposing the quick validation checks' 10s
  // polling deadline; loaded Windows test workers can legitimately exceed it.
...
describe('ProtectDialog', () => {
  ...
  it('setting a modify password produces verifiable writeProtection credentials', async () => {
    ...
    try {
      ...
      expect(result.writeProtection?.spinCount).toBe(100_000)
