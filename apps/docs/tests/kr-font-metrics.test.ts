...
const sans = readWoff2(join(FONTS, 'NiuOfficeSansKR-Regular-subset.woff2'))
const serif = readWoff2(join(FONTS, 'NiuOfficeSerifKR-Regular-subset.woff2'))
...
describe('GenOffice Che Latin KR (fixed-pitch half-width)', () => {
  const che = readWoff2(join(FONTS, 'NiuOfficeCheLatinKR.woff2'))
...
describe('GenOffice Gothic KR (real source metrics, unmodified)', () => {
  const gothicKr = readWoff2(join(FONTS, 'NiuOfficeGothicKR-Regular-subset.woff2'))
