...
const sans = readWoff2(join(FONTS, 'BP-OfficeSansKR-Regular-subset.woff2'))
const serif = readWoff2(join(FONTS, 'BP-OfficeSerifKR-Regular-subset.woff2'))
...
describe('GenOffice Che Latin KR (fixed-pitch half-width)', () => {
  const che = readWoff2(join(FONTS, 'BP-OfficeCheLatinKR.woff2'))
...
describe('GenOffice Gothic KR (real source metrics, unmodified)', () => {
  const gothicKr = readWoff2(join(FONTS, 'BP-OfficeGothicKR-Regular-subset.woff2'))
