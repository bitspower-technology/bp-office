...
import { isAgentProviderConfigured } from './ai/provider-configuration'
...
export function App(): React.JSX.Element {
  ...
  function isAgentConfigured(): boolean {
    return isAgentProviderConfigured(aiSettingsRef.current)
  }
  ...
  useEffect(() => {
    ...
    void window.desktopApi
      .getAiSettings()
      .then((settings) => {
        if (alive) setAiSettingsState(settings)
      })
      .catch(() => {})
    const unsubscribe = window.desktopApi.onAiSettingsChanged((settings) => {
      if (alive) setAiSettingsState(settings)
    })
    return () => {
      alive = false
