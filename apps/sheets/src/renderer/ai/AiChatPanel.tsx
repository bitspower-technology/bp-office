...
export function AiChatPanel({
  ...
  const retryAssistant = (assistantIndex: number): void => {
    const prior = [...chat.slice(0, assistantIndex)]
      .reverse()
      .find((entry) => entry.role === 'user' && entry.text.trim())
    if (prior) onSend(prior.text, prior.attachments ?? [])
  }
          ...
          >
            {entry.role === 'user' ? (
              ...
              <>
                ...
                {entry.isError && index === chat.length - 1 && !aiBusy && (
                  <div className="ai-error-actions">
                    <button className="ai-retry-btn" onClick={() => retryAssistant(index)}>
                      {t('aiRetry')}
                    </button>
                    <button
                      className="ai-settings-btn"
                      onClick={() => void window.desktopApi.openLocalAiSettings()}
                    >
