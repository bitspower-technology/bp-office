...
function localPathsForDroppedFiles(files: File[]): string[] {
  ...
  for (const file of files.slice(0, MAX_DROPPED_FILES)) {
    try {
      ...
      // Ignore forged bridge values; only genuine File objects have a path.
    }
  }
  return paths
}
...
const desktopApi: DesktopApi = {
  ...
  async openLocalAiSettings() {
    await ipcRenderer.invoke(IPC_CHANNELS.aiOpenSettings).catch(() => undefined)
  },
  ...
  async aiToolResult(request) {
    await ipcRenderer.invoke(IPC_CHANNELS.aiToolResult, request)
  },
