...
interface SheetsTabSession {
  ...
  readonly aiStreams: Map<string, SheetsActiveAiStream>
  /// Chunked uploads of large saves' cell edits, pending their save request.
  readonly saveTransfers: SaveEditsTransferStore
}

interface PendingSheetsAiToolResult {
  readonly name: string
  resolve(result: AgentToolResult): void
  reject(error: Error): void
}

interface SheetsActiveAiStream {
  readonly controller: AbortController
  readonly pendingTools: Map<string, PendingSheetsAiToolResult>
}
...
function registerSheetsSession(webContents: WebContents, client: XlsxSidecarClient): void {
  ...
  webContents.once('destroyed', () => {
    ...
    if (entry) {
      for (const stream of entry.aiStreams.values()) {
        stream.controller.abort()
        rejectPendingSheetsAiTools(stream, 'The editor was closed before the tool completed.')
      }
...
function sheetsChatGptProvider() {
...
function rejectPendingSheetsAiTools(stream: SheetsActiveAiStream, reason: string): void {
  for (const pending of stream.pendingTools.values()) pending.reject(new Error(reason))
  stream.pendingTools.clear()
}

function waitForSheetsAiToolResult(
  stream: SheetsActiveAiStream,
  call: AgentToolCall,
...
export function createSheetsView(
  ...
  if (options.openPath) queueWorkbookForView(view.webContents, options.openPath)
...
export function registerSheetsAiIpc(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.aiStream, async (event, input: unknown) => {
    ...
    if (entry.aiStreams.has(requestId)) {
      send({ requestId, type: 'error', error: 'An AI request with this id is already active.' })
      return
    }
    const controller = new AbortController()
    const active: SheetsActiveAiStream = { controller, pendingTools: new Map() }
    entry.aiStreams.set(requestId, active)
    let terminalSent = false
    const sendTerminalAware = (chunk: AiStreamChunk) => {
    ...
    try {
      if (provider === 'chatgpt') {
        await sheetsChatGptProvider().stream(request as unknown as AiStreamRequest, {
          signal: controller.signal,
          onChunk: sendTerminalAware,
          onDynamicToolCall: (call) => waitForSheetsAiToolResult(active, call),
        })
      } else {
        ...
        sendTerminalAware({ requestId, type: 'done' })
      }
    } catch (err) {
      if (controller.signal.aborted) {
        if (!terminalSent) sendTerminalAware({ requestId, type: 'done', stopReason: 'cancelled' })
      } else {
        sendTerminalAware({
          requestId,
          type: 'error',
      ...
      rejectPendingSheetsAiTools(active, 'The AI request ended before the tool completed.')
      if (entry.aiStreams.get(requestId) === active) entry.aiStreams.delete(requestId)
    }
  })

  ipcMain.handle(IPC_CHANNELS.aiToolResult, async (event, input: unknown) => {
    const entry = sessionFor(event)
    const { requestId, result } = aiToolResultRequestSchema.parse(input)
    const stream = entry.aiStreams.get(requestId)
  ...
  ipcMain.handle(IPC_CHANNELS.aiStreamCancel, (event, requestId: unknown) => {
    const entry = sessionFor(event)
    const stream = entry.aiStreams.get(z.string().min(1).max(128).parse(requestId))
    if (!stream) return
    stream.controller.abort()
    rejectPendingSheetsAiTools(stream, 'The AI request was cancelled.')
  })
...
export function registerProjectIpc(): void {
  ...
  ipcMain.handle(
    ...
    (_event, args: { projectId: string; chatId: string; limit?: number }) => {
      return getSheetsProjectStore().loadChat(args.projectId, args.chatId, args.limit ?? 512)
    },
  )
