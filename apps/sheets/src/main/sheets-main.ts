...
import { QueuedWorkbookPaths } from './queued-workbooks'
...
/** Shell-opened paths are one-shot and belong to a specific tab renderer. */
const queuedWorkbookPaths = new QueuedWorkbookPaths()
...
function registerSheetsSession(webContents: WebContents, client: XlsxSidecarClient): void {
  ...
  webContents.once('destroyed', () => {
    ...
    queuedWorkbookPaths.delete(webContents.id)
...
export function hasQueuedWorkbook(webContentsId?: number): boolean {
  return (
    shellQueuedWorkbook ||
    (webContentsId === undefined
      ? queuedWorkbookPaths.has()
      : queuedWorkbookPaths.has(webContentsId))
  )
}
...
export function createSheetsView(
  ...
  if (options.openPath) queuedWorkbookPaths.set(view.webContents.id, options.openPath)
...
export function registerSheetsIpc(): void {
   ...
   */
  ipcMain.handle('sheets:has-queued-workbook', (event) => hasQueuedWorkbook(event.sender.id))

  ipcMain.handle(IPC_CHANNELS.selectWorkbook, async (event) => {
    const entry = sessionFor(event)
    const queuedPath = queuedWorkbookPaths.take(event.sender.id)
    let path = queuedPath ?? forcedWorkbookPath
    if (queuedPath === undefined && shellQueuedWorkbook) {
...
export function registerSheetsAiIpc(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.aiSetSettings, (event, input: unknown) => {
    ...
    if (!event.sender.isDestroyed()) event.sender.send(IPC_CHANNELS.aiSettingsChanged, settings)
  })
  ...
  ipcMain.handle(IPC_CHANNELS.aiChat, async (event, input: unknown) => {
    ...
    if (provider !== 'lmstudio' && !config.model) return { ok: false, error: tm('errNoModel') }
    try {
  ...
  ipcMain.handle(IPC_CHANNELS.aiStream, async (event, input: unknown) => {
    ...
    if (provider !== 'lmstudio' && !config.model) {
...
async function applyMainProcessProxy(): Promise<void> {
  ...
  try {
    await app.whenReady()
    // PAC/rule proxies answer per-host; probe a neutral HTTPS destination.
