...
- ChatGPT subscription access is available through browser OAuth using the
  pinned `@openai/codex` 0.147.0 runtime and an isolated BP-Office data folder.
- The BP-Office mark (black rounded tile with the white monogram, as shipped in
  BP-Office.Setup.0.6.1.exe) is used by the app, installer, portable executable,
  taskbar, onboarding, and AI surfaces.
...
because BP-Office code-signing credentials were not configured. Windows may
