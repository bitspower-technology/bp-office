...
- `apps/docs`, `apps/sheets`, `apps/pdf`, `apps/markdown`, and `apps/shell` —
  the four editors and tabbed shell shipped as BP-Office.
- `apps/slides` — retained standalone source and test workspace. It is not
  built, served, or packaged by the root BP-Office app commands.
...
Run these from the repository root — they regenerate the third-party notices
and build the four shipped editor modules and shell before packaging:
...
| `SERPER_API_KEY`                                         | Supply a Serper key for web/image search instead of keyless fallback   |
...
The `GENOFFICE_*` names are retained compatibility interfaces for existing
automation and user-data isolation; the packaged product name is BP-Office.

LM Studio is configured in **Settings → Local AI** rather than through
environment variables. When it is unavailable, requests surface an actionable
connection message; web search falls back to a keyless backend without a
Serper key.
