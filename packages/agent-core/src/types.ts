...
export interface AgentStreamCallbacks {
  ...
  /**
   * A transport-managed agent can request a local tool result without ending
   * its current model turn. The loop executes the same AgentSkill tool used by
   * ordinary turn-boundary calls and returns a normalized result.
   */
  onToolRequest?(call: AgentToolCall): Promise<AgentToolResult>
  /** Called after a managed tool result has crossed back to the provider. */
  onToolResultSent?(call: AgentToolCall, result: AgentToolResult): void
