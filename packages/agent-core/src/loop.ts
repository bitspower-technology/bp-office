...
export interface CompactionOptions {
  /** History size that triggers compaction (UTF-8 bytes, default 1 MiB, roughly 256K tokens at 4 bytes/token) */
  maxBytes?: number
  /** Recent history kept after compaction (default 384 KiB, roughly 96K tokens, cut at a user boundary) */
...
// Byte budgets approximate model tokens at four bytes per token: compact near
// 256K tokens while preserving roughly the most recent 96K tokens verbatim.
