...
/** Approximate byte cost of one message (text + reasoning + tool inputs/outputs + image base64) */
...
export class AgentLoop<TSnapshot = unknown> {
  ...
  private async beginRun(userMsg: AgentMessage): Promise<void> {
    ...
    // Reasoning echo only matters inside a run's own tool loop. Strip finished
    // runs before compaction so discarded thinking cannot evict useful history.
