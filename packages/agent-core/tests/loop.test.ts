...
describe('AgentLoop', () => {
  ...
  it('the default message cap preserves 200 short conversation rounds', () => {
    const transport = scriptedTransport([])
    const loop = new AgentLoop({ transport, skill: makeSkill(), compaction: false })
    const history: AgentMessage[] = Array.from({ length: 200 }, (_, i) => [
      { role: 'user' as const, text: `question ${i}` },
      { role: 'assistant' as const, text: `answer ${i}` },
    ]).flat()

    loop.restore(history)

    expect(loop.messages).toHaveLength(400)
    expect(loop.messages[0]).toEqual({ role: 'user', text: 'question 0' })
    expect(loop.messages.at(-1)).toEqual({ role: 'assistant', text: 'answer 199' })
  })

  it('the 512-message default cap still trims at a user boundary', () => {
    const transport = scriptedTransport([])
    const loop = new AgentLoop({ transport, skill: makeSkill(), compaction: false })
    const history: AgentMessage[] = Array.from({ length: 257 }, (_, i) => [
      { role: 'user' as const, text: `question ${i}` },
      { role: 'assistant' as const, text: `answer ${i}` },
    ]).flat()
    ...
    expect(loop.messages[0]).toEqual({ role: 'user', text: 'question 1' })
    expect(loop.messages.at(-1)).toEqual({ role: 'assistant', text: 'answer 256' })
  })
})

describe('AgentLoop compaction', () => {
  it('the default byte budget does not compact history merely exceeding the old 256 KiB limit', () => {
    const transport = scriptedTransport([])
    const loop = new AgentLoop({ transport, skill: makeSkill() })
    const oldInstruction = `old instruction ${'x'.repeat(300 * 1024)}`
    ...
    expect(loop.messages).toHaveLength(4)
    expect(loop.messages[0]).toEqual({ role: 'user', text: oldInstruction })
  })

  it('the 1 MiB default compacts while retaining recent turns within the 384 KiB window', () => {
    const transport = scriptedTransport([])
    const loop = new AgentLoop({ transport, skill: makeSkill() })
    const oldInstruction = `old instruction ${'x'.repeat(900 * 1024)}`
    const middleQuestion = `middle question ${'m'.repeat(200 * 1024)}`
    const recentQuestion = `recent question ${'r'.repeat(60 * 1024)}`

    loop.restore([
      { role: 'user', text: oldInstruction },
      { role: 'assistant', text: 'old answer' },
      { role: 'user', text: middleQuestion },
      { role: 'assistant', text: 'middle answer' },
      { role: 'user', text: recentQuestion },
      { role: 'assistant', text: 'recent answer' },
    ])
    ...
    expect(loop.messages.some((m) => 'text' in m && m.text === oldInstruction)).toBe(false)
    expect(loop.messages.some((m) => 'text' in m && m.text === middleQuestion)).toBe(true)
    expect(loop.messages.some((m) => 'text' in m && m.text === recentQuestion)).toBe(true)
  })
