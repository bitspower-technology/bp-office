...
/** One provider-managed turn: dynamic tools run before its final text. */
function dynamicToolTransport(
  calls: AgentToolCall[],
  deliverResults = true,
): AgentTransport & {
  requests: number
  ...
  const transport = {
    requests: 0,
    results: [] as Array<{ id: string; name: string; output: string; isError?: boolean }>,
    cancels: 0,
    stream(request: { tools: unknown[] }, cb: AgentStreamCallbacks) {
      ...
      queueMicrotask(async () => {
        if (request.tools.length === 0) {
          cb.onDelta('managed final answer')
          cb.onDone()
          return
        }
        for (const call of calls) {
          if (stopped) return
          const result = await cb.onToolRequest!(call)
          transport.results.push(result)
          if (deliverResults) cb.onToolResultSent?.(call, result)
        }
        if (stopped) return
        cb.onDelta('managed done')
        cb.onDone()
      })
      return {
        cancel: () => {
...
describe('AgentLoop', () => {
  ...
  it('executes managed mid-turn tools locally and records each pair once', async () => {
    const transport = dynamicToolTransport([
      { id: 'd1', name: 'do_thing', input: { n: 1 } },
      { id: 'd2', name: 'do_thing', input: { n: 2 } },
    ])
    const order: string[] = []
    const onToolExecuted = vi.fn()
    const loop = new AgentLoop({
      ...
      skill: makeSkill((call) => {
        ...
        return { output: `result:${call.id}`, summary: call.id, mutated: true }
      }),
      captureSnapshot: () => 'SNAP',
      events: {
    ...
    loop.run('managed request')
    await flush()
    await flush()

    expect(order).toEqual(['start:d1', 'exec:d1', 'start:d2', 'exec:d2'])
    expect(transport.results.map((result) => result.output)).toEqual(['result:d1', 'result:d2'])
    ...
    const recordedCalls = loop.messages
      .filter((message) => message.role === 'assistant' && message.toolCalls?.length)
      .flatMap((message) => (message.role === 'assistant' ? (message.toolCalls ?? []) : []))
    expect(recordedCalls.map((call) => call.id)).toEqual(['d1', 'd2'])
    expect(loop.messages.at(-1)).toEqual({ role: 'assistant', text: 'managed done' })
  })

  it('delivers one denial at the managed tool cap, then performs a tool-less final turn', async () => {
    const transport = dynamicToolTransport([
      { id: 'one', name: 'do_thing', input: {} },
      { id: 'two', name: 'do_thing', input: {} },
      { id: 'over', name: 'do_thing', input: {} },
      { id: 'never', name: 'do_thing', input: {} },
    ])
    let executed = 0
    const loop = new AgentLoop({
      transport,
    ...
    loop.run('managed limit')
    await flush()
    await flush()
    await flush()

    expect(executed).toBe(2)
    expect(transport.results).toHaveLength(3)
    expect(transport.results[2]).toMatchObject({ id: 'over', isError: true })
    expect(transport.results[2]!.output).toContain('Do not call any more tools')
    expect(transport.cancels).toBe(1)
    expect(transport.requests).toBe(2)
    expect(loop.messages.at(-1)).toEqual({ role: 'assistant', text: 'managed final answer' })
  })

  it('bounds each hostile managed run at the shipped 200-tool cap', async () => {
    const calls = Array.from({ length: 500 }, (_, index) => ({
      id: `call-${index + 1}`,
      name: 'do_thing',
      input: {},
    }))
    const transport = dynamicToolTransport(calls)
    let executed = 0
    ...
    const loop = new AgentLoop({
      ...
      events: { onDone: () => resolveDone?.() },
    })
    const runOnce = (instruction: string) =>
      new Promise<void>((resolve) => {
        resolveDone = resolve
        loop.run(instruction)
      })

    await runOnce('bounded managed run')

    expect(executed).toBe(EDITOR_AGENT_MAX_TURNS)
    expect(transport.results).toHaveLength(EDITOR_AGENT_MAX_TURNS + 1)
    expect(transport.cancels).toBe(1)
    expect(transport.requests).toBe(2)
    expect(loop.busy).toBe(false)

    await runOnce('a new managed run gets a fresh budget')

    expect(executed).toBe(EDITOR_AGENT_MAX_TURNS * 2)
    expect(transport.results).toHaveLength((EDITOR_AGENT_MAX_TURNS + 1) * 2)
    expect(transport.cancels).toBe(2)
    expect(transport.requests).toBe(4)
    expect(loop.busy).toBe(false)
  })

  it('enforces 200 ordinary tool rounds before the tool-less final turn', async () => {
    const toolRounds = Array.from(
      { length: EDITOR_AGENT_MAX_TURNS },
      (_, index) => (cb: AgentStreamCallbacks) => {
        cb.onToolCall({ id: `round-${index + 1}`, name: 'do_thing', input: {} })
        cb.onDone()
      },
    )
    const transport = scriptedTransport([
      ...toolRounds,
      (cb) => {
        cb.onDelta('bounded partial answer')
        cb.onDone()
      },
    ])
    let executed = 0
    ...
    loop.run('bounded ordinary run')
    await completed

    expect(executed).toBe(EDITOR_AGENT_MAX_TURNS)
    expect(transport.requests).toHaveLength(EDITOR_AGENT_MAX_TURNS + 1)
    expect(transport.requests.slice(0, -1).every((request) => request.toolCount > 0)).toBe(true)
  ...
  it('restores up to the default 512-message history at a user boundary', () => {
    const loop = new AgentLoop({ transport: scriptedTransport([]), skill: makeSkill() })
    const messages: AgentMessage[] = Array.from({ length: 600 }, (_, index) =>
      index % 2 === 0
        ? { role: 'user', text: `question ${index}` }
        : { role: 'assistant', text: `answer ${index}` },
    )
    loop.restore(messages)
    expect(loop.messages).toHaveLength(512)
    expect(loop.messages[0]!.role).toBe('user')
  })
  ...
  it('compacts near the default 1 MiB budget and retains roughly 384 KiB verbatim', () => {
    ...
    for (let index = 0; index < 10; index++) {
      messages.push({ role: 'user', text: `question-${index}:${payload}` })
      messages.push({ role: 'assistant', text: `answer-${index}:${payload}` })
    }
    loop.restore(messages)
    expect(loop.messages[0]).toMatchObject({ role: 'user' })
    expect((loop.messages[0] as Extract<AgentMessage, { role: 'user' }>).text).toContain(
      '[Summary of earlier conversation',
    )
    expect(loop.messages).toHaveLength(6)
    expect(JSON.stringify(loop.messages)).toContain('question-8')
    expect(JSON.stringify(loop.messages)).toContain('question-9')
    expect(JSON.stringify(loop.messages)).not.toContain(`question-0:${payload}`)
  })
...
describe('AgentLoop compaction', () => {
  it('accounts for the pending user/context message when deciding to compact', async () => {
    ...
    const pendingInstruction = 'third request'.padEnd(300, 'c')
    const transport = scriptedTransport([
      (cb) => {
        cb.onDelta(firstAnswer)
        cb.onDone()
      },
      (cb) => {
        cb.onDelta(secondAnswer)
        cb.onDone()
      },
      (cb) => {
        cb.onDelta('summary of the first turn')
        cb.onDone()
      },
      (cb) => {
        cb.onDelta('continued after pending-aware compaction')
        cb.onDone()
      },
    ])
    const loop = new AgentLoop({
      transport,
      skill: makeSkill(),
      compaction: { maxBytes: 700, keepRecentBytes: 100 },
    })
    ...
    expect(JSON.stringify(loop.messages)).not.toContain(firstAnswer)
    expect(JSON.stringify(loop.messages)).toContain(pendingInstruction)
  })

  it('rejects an oversized current editor context before any model request', async () => {
    const transport = scriptedTransport([])
    const skill = makeSkill()
    skill.buildContext = () => 'x'.repeat(1024 * 1024)
    const onError = vi.fn()
    const loop = new AgentLoop({ transport, skill, events: { onError } })

    loop.run('use the current document context')
    await flush()
  ...
  it('over budget before run: folds old conversation via LLM summary; new request carries the summary, not the originals', async () => {
    ...
    const transport = scriptedTransport([
      ...
      // The pending 2nd run tips the projected request over budget, so the
      // single oversized prior turn is folded before another model request.
      (cb) => {
  ...
  it('counts interleaved reasoning when deciding to compact a long tool loop', async () => {
    const output = 'z'.repeat(2_000)
    const script = Array.from({ length: 3 }, (_, index) => (cb: AgentStreamCallbacks) => {
      cb.onReasoning?.('r'.repeat(1_000))
      cb.onToolCall({ id: `reasoning-${index}`, name: 'do_thing', input: {} })
      cb.onDone()
    })
    script.push((cb) => {
      cb.onDelta('done')
    ...
    const loop = new AgentLoop({
      ...
      skill: makeSkill(() => ({ output, summary: 'output' })),
      compaction: { maxBytes: 8_500, keepRecentBytes: 2_000, disableLlmSummary: true },
    })

    loop.run('work')
    for (let index = 0; index < 4; index++) await flush()

    const tools = loop.messages.filter((message) => message.role === 'tool')
    expect(tools[0]!.results[0]!.output).toContain('output truncated')
    expect(tools[2]!.results[0]!.output).toBe(output)
    const thinking = loop.messages.filter((message) => message.role === 'assistant')
    expect(thinking[0]!.reasoning).toHaveLength(1_000)
  })

  it('drops completed reasoning before evaluating the next run compaction budget', async () => {
    const transport = scriptedTransport([
      (cb) => {
        cb.onReasoning?.('r'.repeat(2_000))
        cb.onToolCall({ id: 'reasoning', name: 'do_thing', input: {} })
        cb.onDone()
      },
      (cb) => {
        cb.onDelta('done')
    ...
    expect(transport.requests).toHaveLength(3)
    expect(loop.messages[0]).toEqual({ role: 'user', text: 'first\n\nCTX' })
    expect(loop.messages.at(-1)).toEqual({ role: 'assistant', text: 'continued' })
    expect(JSON.stringify(loop.messages)).not.toContain('auto-compacted')
  })
