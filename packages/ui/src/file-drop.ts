 ...
 * the bubble phase and honor defaultPrevented, so attachment/image drop zones
...
export function installFileDropOpen(
  ...
  const onDrop = (event: DragEvent): void => {
    // An inner attachment/image target may have claimed the drop. Always clear
    // our visual state, but never open that target's files as documents.
