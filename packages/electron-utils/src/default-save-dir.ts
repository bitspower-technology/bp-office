...
/// <Documents>/GenOffice; fresh defaults now use <Documents>/BP-Office and are
/// user-configurable via the `defaultSaveDir` key in userData/app-settings.json
/// (set from the home screen's account menu).
...
export function configuredDefaultSaveDir(app: PathProvider): string {
  ...
  const fallback = join(app.getPath('documents'), 'BP-Office')
