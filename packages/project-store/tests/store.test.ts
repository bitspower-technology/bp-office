...
describe('appendChatMessage + loadChat', () => {
  ...
  it('defaults to the most recent 512 messages', () => {
    for (let i = 0; i < 513; i++) {
      store.appendChatMessage('default', 'chatDefaultLimit', {
        role: 'assistant',
        text: `msg${i}`,
      })
    }
    const msgs = store.loadChat('default', 'chatDefaultLimit')
    expect(msgs).toHaveLength(512)
    expect(msgs[0].text).toBe('msg1')
    expect(msgs[511].text).toBe('msg512')
  })
