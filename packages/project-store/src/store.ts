...
export class ProjectStore {
  ...
  /**
   * Reads the most recent `limit` messages (512 by default, in ascending seq order).
   * A bad JSONL line is skipped without crashing.
   */
  loadChat(projectId: string, chatId: string, limit = 512): ChatMessage[] {
   ...
   * records and may outlive files deleted or moved outside BP-Office.
   */
  ...
  getProjectTimeline(projectId: string, limit = 20): TimelineEntry[] {
    ...
    for (const { chatId } of chats) {
      ...
      const msgs = this.loadChat(projectId, chatId, 512)
