...
const enabled = process.env.BP_OFFICE_CHATGPT_SMOKE === '1'
...
describe.runIf(enabled)('official Codex app-server smoke', () => {
  it('initializes against a fresh isolated home without starting login', async () => {
    const root = await mkdtemp(join(tmpdir(), 'bpoffice-chatgpt-smoke-'))
    const executablePath =
      process.env.BP_OFFICE_CODEX_SMOKE_PATH ?? resolveInstalledChatGptExecutable()
