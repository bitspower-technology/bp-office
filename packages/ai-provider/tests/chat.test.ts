...
describe('chatForProvider', () => {
  ...
  it('LM Studio discovers the model and omits blank optional authentication', async () => {
    const fetchMock = vi
      .fn()
    ...
    vi.stubGlobal('fetch', fetchMock)
    const result = await chatForProvider('lmstudio', { model: '' }, 'sys', 'hi')
  ...
  it('LM Studio sends a configured optional token', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(
        jsonResponse({
