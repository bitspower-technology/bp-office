...
describe('streamForProvider: anthropic', () => {
  ...
  it('emits the text of a complete JSON message sent instead of an SSE stream', async () => {
    ...
    // dissolve into an empty "successful" turn (error-bearing bodies throw below)
    vi.stubGlobal(
      'fetch',
  ...
  it('throws on an empty or error-bearing JSON body sent instead of an SSE stream', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(jsonResponse({ error: { message: 'Quota exceeded' } })),
    )
    const { cb } = collector()
    await expect(
      streamForProvider('anthropic', { apiKey: 'k', model: 'm' }, 'sys', [], [], 100, cb),
...
describe('streamForProvider: lmstudio', () => {
  it('uses the default OpenAI-compatible endpoint without an Authorization header', async () => {
    const fetchMock = vi
      .fn()
      ...
      .mockResolvedValueOnce(okResponse(sseStream(['data: [DONE]'])))
    vi.stubGlobal('fetch', fetchMock)
    const { cb } = collector()
    await streamForProvider('lmstudio', { model: 'local-model' }, 'sys', [], [], 100, cb).catch(
      () => {},
    )
    expect(fetchMock).toHaveBeenCalledWith(
  ...
  it('normalizes a configured server root and sends an optional API token', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(
        jsonResponse({
  ...
  it('resolves automatic mode before streaming', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(
        jsonResponse({
          models: [
            {
              type: 'llm',
              key: 'auto-tool-model',
              display_name: 'Auto Tool Model',
              loaded_instances: [{ id: 'auto-tool-model' }],
    ...
    await streamForProvider('lmstudio', { model: '' }, 'sys', [], [], 100, cb)

    expect(deltas).toEqual(['ok'])
    expect(JSON.parse(fetchMock.mock.calls[1]![1].body as string)).toMatchObject({
      model: 'auto-tool-model',
    })
  })
})
...
describe('streamForProvider: 200 + non-stream JSON instead of SSE', () => {
  ...
  it('anthropic route emits a complete JSON reply', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        json({
    ...
    const { deltas, cb } = collector()
    await streamForProvider('anthropic', { apiKey: 'k', model: 'm' }, 'sys', [], [], 100, cb)
    expect(deltas).toEqual(['Complete reply'])
  })

  it('gemini route emits a complete JSON reply', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        json({
          candidates: [
            {
              content: {
                parts: [{ text: 'Complete Gemini reply' }],
              },
            },
          ],
    ...
    const { deltas, cb } = collector()
    await streamForProvider('gemini', { apiKey: 'k', model: 'm' }, 'sys', [], [], 100, cb)
    expect(deltas).toEqual(['Complete Gemini reply'])
  })

  it('openai route emits a complete JSON reply', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        json({
          choices: [{ message: { role: 'assistant', content: 'Complete OpenAI reply' } }],
    ...
    const { deltas, cb } = collector()
    await streamForProvider('openai', { apiKey: 'k', model: 'm' }, 'sys', [], [], 100, cb)
    expect(deltas).toEqual(['Complete OpenAI reply'])
  })

  it('a service notice is emitted as the reply text instead of an empty turn', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        json({
