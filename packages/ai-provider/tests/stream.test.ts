...
describe('streamForProvider: anthropic', () => {
  ...
  it('emits the text of a complete JSON message sent instead of an SSE stream', async () => {
    ...
    // dissolve into an empty successful turn.
    vi.stubGlobal(
      'fetch',
...
describe('streamForProvider: lmstudio', () => {
  it('rejects a missing API key before any request', async () => {
    ...
    const { cb } = collector()
    await expect(
      streamForProvider('lmstudio', { model: 'local-model' }, 'sys', [], [], 100, cb),
    ).rejects.toThrow(/requires an API key/)
    expect(fetchMock).not.toHaveBeenCalled()
  })

  it('normalizes a custom root and sends the API key to discovery and chat', async () => {
    const fetchMock = vi
      .fn()
  ...
  it('resolves automatic mode before streaming', async () => {
    ...
    await streamForProvider('lmstudio', { apiKey: 'client-key', model: '' }, 'sys', [], [], 100, cb)
...
describe('streamForProvider: complete JSON instead of SSE', () => {
...
describe('streamForProvider: interleaved-thinking reasoning', () => {
  ...
  it('surfaces reasoning deltas and echoes stored reasoning for thinking families', async () => {
    const fetchMock = vi.fn<(url: string, init: RequestInit) => Promise<Response>>((url) =>
      Promise.resolve(
        url.endsWith('/api/v1/models')
          ? jsonResponse({ models: [{ key: 'deep-seek-v4-flash', type: 'llm' }] })
          : reasoningTurn(),
      ),
    )
    vi.stubGlobal('fetch', fetchMock)
    ...
    await streamForProvider(
      'lmstudio',
      { apiKey: 'client-key', model: 'deep-seek-v4-flash' },
      'sys',
      toolLoopMessages,
      [],
      100,
    ...
    const [, request] = fetchMock.mock.calls.find(([url]) => url.endsWith('/chat/completions'))!
