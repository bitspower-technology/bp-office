...
describe('listLmStudioModels', () => {
  it('parses native metadata, filters embeddings, and omits auth when no token is set', async () => {
    ...
    await expect(
      listLmStudioModels({ model: '', baseUrl: 'http://localhost:1234' }),
    ).resolves.toEqual([
      {
        id: 'tool-model',
...
describe('selectLmStudioModel', () => {
  ...
  it('resolves automatic selection and rejects embedding-only model lists', async () => {
    ...
    await expect(resolveLmStudioModel({ model: '' })).resolves.toBe('tool-model')
    await expect(resolveLmStudioModel({ model: '' })).rejects.toThrow(/no chat models/i)
  })
})
...
describe('checkLmStudioStatus', () => {
  it('returns connected with the selected discovered model', async () => {
    ...
    await expect(checkLmStudioStatus({ model: '' })).resolves.toMatchObject({
      state: 'connected',
  ...
  it('times out an unresponsive probe when no caller signal is supplied', async () => {
    ...
    const status = checkLmStudioStatus({ model: '' })
