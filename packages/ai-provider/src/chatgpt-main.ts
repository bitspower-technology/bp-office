...
const CHATGPT_DISABLED_CODEX_FEATURES = Object.freeze(
  Object.fromEntries(CHATGPT_DENIED_CODEX_FEATURES.map((feature) => [feature, false])) as Record<
    (typeof CHATGPT_DENIED_CODEX_FEATURES)[number],
    false
  >,
)
...
export const CHATGPT_RESTRICTED_THREAD_CONFIG = Object.freeze({
  ...
  features: CHATGPT_DISABLED_CODEX_FEATURES,
  include_apps_instructions: false,
  include_collaboration_mode_instructions: false,
  include_environment_context: false,
  include_permissions_instructions: false,
...
const RESTRICTED_BASE_INSTRUCTIONS =
  ...
  'connectors, computer use, code mode, image generation, or subagents. The isolated working ' +
...
const SHARED_CHATGPT_SERVICE = Symbol.for('bpoffice.chatgpt-provider-service')
...
export function resolveChatGptExecutable(options: ChatGptAppServerOptions): string {
  ...
  const envName = options.executableEnvVar ?? 'BP_OFFICE_CODEX_PATH'
...
function serializeMessages(messages: AgentMessage[]): Array<JsonRecord> {
  ...
  const payload = safeJson({
    format: 'bpoffice-agent-history-v1',
...
function sharedOptionsKey(options: ChatGptAppServerOptions): string {
  return JSON.stringify({
    ...
    executableEnvVar: options.executableEnvVar ?? 'BP_OFFICE_CODEX_PATH',
