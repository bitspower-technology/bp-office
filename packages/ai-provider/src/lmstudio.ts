...
function parseModels(payload: unknown, source: 'native' | 'openai'): LmStudioModel[] {
  if (!isRecord(payload)) throw new Error('LM Studio returned an invalid model list')
...
async function requireOk(response: Response): Promise<Response> {
  ...
  throw new LmStudioHttpError(
    response.status,
    `LM Studio HTTP ${response.status}${detail ? `: ${detail}` : ''}`,
  )
}
...
export async function resolveLmStudioModel(
  ...
  if (!selected) {
    throw new Error(
      'LM Studio is connected, but no chat models are available. Load an LLM in LM Studio and try again.',
    )
  }
  return selected
}
