...
NiuOffice opens, edits, and saves documents locally. It contains no usage
...
NiuOffice does not provide network web or image search, Genspark account or
...
application user-data directory. This file is not encrypted by NiuOffice. An
OEM distributor that requires keychain-backed or managed secret storage must
implement and disclose that separately. No API key is built into NiuOffice.
...
Automatic updates are disabled in the NiuOffice OEM source template. If a
distributor enables them, checking for or downloading an update contacts that
distributor's configured public release infrastructure. Opening an external
link uses the system browser and contacts that link's destination. Standard
network metadata, including an IP address, is necessarily visible to those
services.

## Data not collected by NiuOffice telemetry

Because NiuOffice ships no telemetry pipeline, it does not send analytics
