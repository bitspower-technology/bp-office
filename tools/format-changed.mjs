...
const prettierExecutable = process.execPath
const prettierEntrypoint = join(repoRoot, 'node_modules', 'prettier', 'bin', 'prettier.cjs')
const prettierMode = mode === '--write' ? '--write' : '--check'
const maxArgumentLength = process.platform === 'win32' ? 6_000 : 120_000
const fileBatches = []
let currentBatch = []
let currentLength = 0
...
for (const file of changedFiles) {
  ...
  if (currentBatch.length > 0 && currentLength + argumentLength > maxArgumentLength) {
...
for (const batch of fileBatches) {
  const result = spawnSync(
    prettierExecutable,
    [prettierEntrypoint, prettierMode, '--ignore-unknown', '--', ...batch],
    {
      cwd: repoRoot,
      stdio: 'inherit',
      env: {
