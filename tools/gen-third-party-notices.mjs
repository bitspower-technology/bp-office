...
const PRODUCT_CONFIG = JSON.parse(readFileSync(join(ROOT, 'branding/product.json'), 'utf8'))
...
/** Runtimes that are always present but never imported by a bundled specifier. */
const IMPLICIT = [
  'electron',
  ...(PRODUCT_CONFIG.features?.chatgptSubscription ? ['@openai/codex'] : []),
]

/**
 * Packages copied into the installer verbatim by electron-builder rather than
 * bundled are invisible to the import scan.
 */
...
// The repository license intentionally carries GenOffice's historical
// Mainfunc application notice in its Apache appendix. Codex is a separate
// OpenAI work, so restore the canonical Apache appendix placeholder before
// reproducing the license in the Codex package sections.
const APACHE_TEMPLATE_COPYRIGHT = '   Copyright [yyyy] [name of copyright owner]'
const PROJECT_APACHE_COPYRIGHT = '   Copyright 2026 Mainfunc, Inc.'
const projectApacheLicense = readFileSync(join(ROOT, 'LICENSE'), 'utf8').trim()
if (!projectApacheLicense.includes(PROJECT_APACHE_COPYRIGHT)) {
  throw new Error('Root LICENSE no longer contains the expected historical copyright example')
}
const canonicalApacheLicense = projectApacheLicense.replace(
  PROJECT_APACHE_COPYRIGHT,
  APACHE_TEMPLATE_COPYRIGHT,
)

const CODEX_NOTICE =
  'Copyright OpenAI. Source: https://github.com/openai/codex\n' +
...
const NOTE = {
  ...
  '@openai/codex': CODEX_NOTICE,
  '@openai/codex-darwin-arm64': CODEX_NOTICE,
  '@openai/codex-darwin-x64': CODEX_NOTICE,
  '@openai/codex-linux-arm64': CODEX_NOTICE,
  '@openai/codex-linux-x64': CODEX_NOTICE,
  '@openai/codex-win32-arm64': CODEX_NOTICE,
  '@openai/codex-win32-x64': CODEX_NOTICE,
}
...
let out = `BP Office — Third-Party Software Notices
