...
test.describe('first-run onboarding', () => {
  test('fresh install walks all slides and persists the seen flag', async () => {
    ...
    try {
      ...
      const localAiPanel = page.locator('.onb-slide.active .onb-local-ai-panel')
      await expect(localAiPanel).toBeVisible()
      await expect(localAiPanel).toContainText('Local AI runs through LM Studio')
