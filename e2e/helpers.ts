...
/**
 * Begin an Explorer/Finder-like file drag over an Electron renderer.
 *
 ...
 * the same path-backed `File` objects as an OS drag. Keeping the drag open lets
 * callers assert drag-over UI before completing the drop.
 */
...
/** Dispatch a complete Explorer/Finder-like local file drag/drop gesture. */
...
/** Create scratch files for drag/drop E2E without adding generated fixtures to the repository. */
export async function createFileDropFixtureDir(): Promise<string> {
  return mkdtemp(join(tmpdir(), 'niuoffice-e2e-drop-'))
}
...
export async function removeFileDropFixtureDir(dir: string): Promise<void> {
  ...
  if (!insideTemp || !basename(candidate).startsWith('niuoffice-e2e-drop-')) {
