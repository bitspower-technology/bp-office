...
export interface AiSettingsSeed {
  provider?: string
...
interface LaunchOptions {
  ...
  /** pre-seed ai-settings.json in the isolated userData directory */
  aiSettings?: AiSettingsSeed
...
export async function launchShell(options: LaunchOptions): Promise<LaunchedApp> {
  ...
  if (options.aiSettings) {
    await writeFile(join(userDataDir, 'ai-settings.json'), JSON.stringify(options.aiSettings))
  }
...
export interface LocalFileDrag {
  /** Complete the native file drop at the target coordinates. */
  drop(): Promise<void>
  /** Abort an unfinished drag. Safe to call after drop or more than once. */
  cancel(): Promise<void>
}

/**
 * Begin a native-like file drag over an Electron renderer.
 *
 * A JavaScript-created `File` has no operating-system backing path, so
 * Electron's `webUtils.getPathForFile()` correctly returns an empty string for
 * it. CDP's drag payload accepts real filenames and makes Chromium construct
 * the same path-backed `File` objects as a drag from Finder/Explorer. Keeping
 * the drag open lets a test assert drag-over UI before calling `drop()`.
 */
export async function beginLocalFileDrag(
  page: Page,
  filePaths: readonly string[],
  target: Locator | string = 'body',
): Promise<LocalFileDrag> {
  if (filePaths.length === 0) throw new Error('A local file drag needs at least one path')
  const files = filePaths.map((path) => resolve(path))
  for (const file of files) {
    const info = await stat(file).catch(() => null)
    if (!info?.isFile()) throw new Error(`Cannot drag a missing or non-file path: ${file}`)
  }

  const locator = typeof target === 'string' ? page.locator(target) : target
  await locator.waitFor({ state: 'visible' })
  const box = await locator.boundingBox()
  if (!box) throw new Error('Cannot drag files to an element without a bounding box')
  ...
  const finish = async (type: 'drop' | 'dragCancel') => {
    if (finished) return
    finished = true
    try {
  ...
  return {
    drop: () => finish('drop'),
    cancel: () => finish('dragCancel'),
  }
}

/** Dispatch a complete native-like local file drag/drop gesture. */
export async function dropLocalFiles(
  page: Page,
  filePaths: readonly string[],
  target: Locator | string = 'body',
): Promise<void> {
  const drag = await beginLocalFileDrag(page, filePaths, target)
  await drag.drop()
}

/** Create scratch files for drag/drop E2E without placing generated fixtures in the repository. */
export async function createFileDropFixtureDir(): Promise<string> {
  return mkdtemp(join(tmpdir(), 'bpoffice-e2e-drop-'))
}

/** Remove only a fixture directory created by createFileDropFixtureDir(). */
export async function removeFileDropFixtureDir(dir: string): Promise<void> {
  const candidate = resolve(dir)
  const scratchRoot = resolve(tmpdir())
  const insideTemp = candidate.toLowerCase().startsWith(`${scratchRoot.toLowerCase()}${sep}`)
  if (!insideTemp || !basename(candidate).startsWith('bpoffice-e2e-drop-')) {
    throw new Error(`Refusing to remove non-file-drop fixture directory: ${candidate}`)
  }
  await rm(candidate, { recursive: true, force: true })
}

/** Remove only launcher-created scratch state after its Electron process has closed. */
export async function removeScratchUserData(userDataDir: string): Promise<void> {
  const candidate = resolve(userDataDir)
  const scratchRoot = resolve(tmpdir())
  const insideTemp = candidate.toLowerCase().startsWith(`${scratchRoot.toLowerCase()}${sep}`)
  if (!insideTemp || !basename(candidate).startsWith('genoffice-e2e-')) {
    throw new Error(`Refusing to remove non-E2E userData directory: ${candidate}`)
  }
  await rm(candidate, { recursive: true, force: true })
}
