...
import { readZipText, writeZipEntry } from './zip-archive'
...
function sheetXml(workbookPath: string): Promise<string> {
  return readZipText(workbookPath, 'xl/worksheets/sheet1.xml')
}
...
test.describe('sheets: ribbon batch-3 features', () => {
  test('page breaks + preview, workbook protection, allow edit ranges, theme', async () => {
    ...
    await writeZipEntry(workbook, 'xl/theme/theme1.xml', MINIMAL_THEME)
    ...
    try {
      ...
      await expect(async () => {
        ...
        expect(await readZipText(workbook, 'xl/workbook.xml')).toContain('lockStructure="1"')
        const theme = await readZipText(workbook, 'xl/theme/theme1.xml')
