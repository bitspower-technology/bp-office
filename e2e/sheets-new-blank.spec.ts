...
import { readZipText } from './zip-archive'
...
test.describe('sheets: new blank workbook', () => {
  test('quick-create writes a backing file and saves the first edit', async () => {
    ...
    try {
      ...
      // keep the auto-created workbook out of the real ~/Documents/NiuOffice
      ...
      const saveDir = join(scratch, 'NiuOffice')
      ...
      await expect(async () => {
        const xml = await readZipText(workbook, 'xl/worksheets/sheet1.xml')
