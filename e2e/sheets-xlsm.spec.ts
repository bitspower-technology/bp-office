...
import { readZipEntry, readZipText } from './zip-archive'
...
test.describe('sheets: macro-enabled workbook (.xlsm)', () => {
  test('opens, edits, and saves with the VBA project preserved verbatim', async () => {
    ...
    const vbaBefore = await readZipEntry(workbook, 'xl/vbaProject.bin')
    ...
    try {
      ...
      await expect(async () => {
        expect(await readZipText(workbook, 'xl/worksheets/sheet1.xml')).toContain('MacroSafe')
      }).toPass({ timeout: 15_000 })

      // macros are never executed, but they must survive the save byte-for-byte
      expect((await readZipEntry(workbook, 'xl/vbaProject.bin')).equals(vbaBefore)).toBe(true)
      const contentTypes = await readZipText(workbook, '[Content_Types].xml')
      ...
      const rels = await readZipText(workbook, 'xl/_rels/workbook.xml.rels')
